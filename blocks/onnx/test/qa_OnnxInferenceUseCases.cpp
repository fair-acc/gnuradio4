...
const boost::ut::suite<"OnnxInference use-case (a): 1:1 elementwise"> useCaseATests = [] {
    "affine fixture loads and self-configures from metadata"_test = [] {
        ...
        block.model_path = MODEL_AFFINE_N64_PATH;
        block.start();
        if (!block.isModelLoaded()) {
            expect(false) << "tracked fixture failed to load: " << MODEL_AFFINE_N64_PATH;
            return;
        }
        expect(block.model_io_shape.value == std::vector<gr::Size_t>{1U, 1U, 64U, 1U, 64U}) << "input [1,1,64] + output [1,64]";
    ...
    "graph run computes y = 2x + 1, one output per input"_test = [] {
        ...
        auto&     inference = graph.emplaceBlock<OnnxInference<float>>({{"model_path", std::string(MODEL_AFFINE_N64_PATH)}, {"normalise_mode", "None"}, {"resample_mode", "None"}});
...
#endif // MODEL_AFFINE_N64_PATH

#ifdef MODEL_FRAME_MEAN_N64_M4_PATH
...
const boost::ut::suite<"OnnxInference use-case (b): N:1 decimating"> useCaseBTests = [] {
    "frame-mean fixture loads and self-configures history_depth = 4"_test = [] {
        OnnxInference<float> block;
        block.model_path     = MODEL_FRAME_MEAN_N64_M4_PATH;
        block.history_stride = 4U;
        block.start();
        if (!block.isModelLoaded()) {
            expect(false) << "tracked fixture failed to load: " << MODEL_FRAME_MEAN_N64_M4_PATH;
            return;
        }
    ...
    "graph run decimates 8 inputs to 2 window means"_test = [] {
        ...
        auto&     inference = graph.emplaceBlock<OnnxInference<float>>({{"model_path", std::string(MODEL_FRAME_MEAN_N64_M4_PATH)}, {"history_stride", 4U}, {"normalise_mode", "None"}, {"resample_mode", "None"}});
        auto&     sink      = graph.emplaceBlock<CollectSink<gr::DataSet<float>>>();
        if (!inference.isModelLoaded()) {
            expect(false) << "tracked fixture failed to load: " << MODEL_FRAME_MEAN_N64_M4_PATH;
            return;
...
#endif // MODEL_FRAME_MEAN_N64_M4_PATH

#ifdef MODEL_FRAME_DELTA_N64_M8_PATH
...
const boost::ut::suite<"OnnxInference use-case (c): sliding-window history"> useCaseCTests = [] {
    "frame-delta fixture loads and self-configures history_depth = 8"_test = [] {
        OnnxInference<float> block;
        block.model_path = MODEL_FRAME_DELTA_N64_M8_PATH;
        block.start();
        if (!block.isModelLoaded()) {
            expect(false) << "tracked fixture failed to load: " << MODEL_FRAME_DELTA_N64_M8_PATH;
            return;
        }
    ...
    "graph run passes 7 warm-up frames through, then emits newest - oldest per input"_test = [] {
        ...
        auto&     inference = graph.emplaceBlock<OnnxInference<float>>({{"model_path", std::string(MODEL_FRAME_DELTA_N64_M8_PATH)}, {"normalise_mode", "None"}, {"resample_mode", "None"}});
        auto&     sink      = graph.emplaceBlock<CollectSink<gr::DataSet<float>>>();
        if (!inference.isModelLoaded()) {
            expect(false) << "tracked fixture failed to load: " << MODEL_FRAME_DELTA_N64_M8_PATH;
            return;
...
#endif // MODEL_FRAME_DELTA_N64_M8_PATH
...
const boost::ut::suite<"OnnxInference use-case (d): multi-output"> useCaseDTests = [] {
    "mean/rms fixture loads and exposes one meta_out port"_test = [] {
        OnnxInference<float> block;
        block.model_path = MODEL_MEAN_RMS_N64_PATH;
        block.start();
        if (!block.isModelLoaded()) {
            expect(false) << "tracked fixture failed to load: " << MODEL_MEAN_RMS_N64_PATH;
            return;
        }
    ...
    "graph run routes mean to out and rms to meta_out#0"_test = [] {
        ...
        gr::Graph graph;
        auto&     source    = graph.emplaceBlock<RampFrameSource>({{"n_frames", 3U}});
        auto&     inference = graph.emplaceBlock<OnnxInference<float>>({{"model_path", std::string(MODEL_MEAN_RMS_N64_PATH)}, {"normalise_mode", "None"}, {"resample_mode", "None"}});
