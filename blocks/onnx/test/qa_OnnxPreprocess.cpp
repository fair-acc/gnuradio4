...
const boost::ut::suite<"OnnxPreprocess LogMAD"> logMadTests = [] {
    ...
    "LogMAD handles negative values"_test = [] {
    ...
    "LogMAD synthetic spectrum keeps peaks above the noise floor"_test = [] {
        std::vector<float> spectrum(1024, 0.1f);
        for (const auto& [centre, amplitude] : {std::pair{200.f, 50.f}, std::pair{500.f, 80.f}, std::pair{800.f, 40.f}}) {
            for (std::size_t i = 0; i < spectrum.size(); ++i) {
                float x = static_cast<float>(i) - centre;
                spectrum[i] += amplitude * std::exp(-0.5f * x * x / 25.f);
            }
        }

        std::vector<float> output(1024);
        OnnxPreprocess<float>::normaliseLogMAD(spectrum, output);
...
const boost::ut::suite<"OnnxPreprocess Expression"> exprTests = [] {
    "simple pass-through expression"_test = [] {
        ...
        expect(pp.configure(NormaliseMode::Expression, "vecOut := vecIn").has_value());
    ...
    "min-max via expression"_test = [] {
        OnnxPreprocess<float> pp;
        expect(pp.configure(NormaliseMode::Expression, "vecOut := (vecIn - min_val) / (max_val - min_val + 1e-10)").has_value());
    ...
    "z-score via expression"_test = [] {
        OnnxPreprocess<float> pp;
        expect(pp.configure(NormaliseMode::Expression, "vecOut := (vecIn - mean_val) / (std_val + 1e-10)").has_value());
    ...
    "uses pre-computed statistics"_test = [] {
        ...
        expect(pp.configure(NormaliseMode::Expression, "vecOut := (vecIn - median) / (1.4826 * mad + 1e-10)").has_value());
    ...
    "invalid expression returns error"_test = [] {
        OnnxPreprocess<float> pp;
        auto                  result = pp.configure(NormaliseMode::Expression, "vecOut := this_is_not_valid_syntax @#$");
    ...
    "large input vector"_test = [] {
        OnnxPreprocess<float> pp;
        expect(pp.configure(NormaliseMode::Expression, "vecOut := vecIn / max_val").has_value());
...
const boost::ut::suite<"OnnxPreprocess dispatch"> dispatchTests = [] {
    "dispatch None mode copies input"_test = [] {
        OnnxPreprocess<float> pp;
        expect(pp.configure(NormaliseMode::None).has_value());
    ...
    "dispatch LogMAD mode"_test = [] {
        OnnxPreprocess<float> pp;
        expect(pp.configure(NormaliseMode::LogMAD).has_value());
    ...
    "dispatch MinMax mode"_test = [] {
        OnnxPreprocess<float> pp;
        expect(pp.configure(NormaliseMode::MinMax).has_value());
    ...
    "dispatch ZScore mode"_test = [] {
        OnnxPreprocess<float> pp;
        expect(pp.configure(NormaliseMode::ZScore).has_value());
