...
private:
    ...
    [[nodiscard]] static NoiseEstimate estimateNoise(std::span<const float> data) {
    ...
    [[nodiscard]] static float estimateProminence(std::span<const float> data, std::size_t peakIdx) {
    ...
    [[nodiscard]] static WidthEstimate estimateWidth(std::span<const float> data, std::size_t peakIdx) {
        const float peakVal = data[peakIdx];
        const auto  n       = data.size();
