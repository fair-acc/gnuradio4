 ...
 *   TIn  - input port type: DataSet<T>, Tensor<T>
 *   TOut - output port type: DataSet<T>, Tensor<T>
 *
 ...
 *
 * History mode (DataSet<T> -> DataSet<T>, history_depth M > 1): the inference window
 * always covers the newest M frames. With history_stride 1 the window slides per input
 * and the block stays 1:1; while the window first fills, each input passes through
 * unchanged — the module-wide warm-up contract shared with OnnxPeakDetector, no empty
 * placeholders. With history_stride S > 1 the block genuinely decimates S:1 through the
 * framework's Resampling machinery (input_chunk_size = S, output_chunk_size = 1): one
 * output per S inputs, so S < M overlaps consecutive windows and S > M skips frames.
 *
 * Multi-output models: `out` always carries model output 0; outputs 1..n-1 are exposed
 * on the optional `meta_out` ports (one Tensor<T> per inference). The full named output
 * set is only requested from ORT when at least one meta_out port is connected, so
 * single-output use keeps the pruned fast path.
 *
 * Model configuration: each overridable initializer of the model is exposed as an
 * optional scalar `config_in` port (latching the last received value, broadcast to the
 * initializer's element count) and can also be set by name via the `model_overrides`
 * map. Precedence: fed config_in port > model_overrides entry > model-baked default.
 * A config feed may start and stop at any time; its end-of-stream does not end the block.
 * A map entry that cannot be applied — unknown initializer name, element-count mismatch,
 * or unsupported value type — is reported through the block's error policy instead of
 * being silently ignored.
 *
 * Input normalisation: `normalise_mode` selects None/LogMAD/MinMax/ZScore/Expression.
 * Expression evaluates the ExprTk `normalise_expr` over the resampled input (`vecIn` ->
 * `vecOut`, with n/median/mad/min_val/max_val/mean_val/std_val pre-computed). A model can
 * request its normalisation via the `normalise_mode` (+ `normalise_expr`) ONNX metadata.
 *
 * Frames forwarded unchanged (no model loaded, history warm-up, ErrorPolicy::Passthrough)
 * carry meta_information[0]["onnx_passthrough"] = true on DataSet outputs; bare Tensor<T>
 * outputs have no metadata and stay unmarked.
 */
...
Supports DataSet<T> and Tensor<T> input/output with configurable
preprocessing (resample; None/LogMAD/MinMax/ZScore/ExprTk-Expression
normalisation), history-based batched inference (1:1 sliding or genuine
N:1 decimation), optional meta_out ports for model outputs 1..n-1 and
optional config_in ports plus a model_overrides map for overridable
model initializers.)"">;
    ...
    GR_MAKE_REFLECTABLE(OnnxInference, in, config_in, out, meta_out, model_path, error_policy, execution_provider, resample_mode, normalise_mode, clip_min, clip_max, normalise_expr, history_depth, history_stride, model_overrides, model_io_shape, meta_output_names, config_input_names, available_providers, passthrough_reason);
    ...
    // last value received per config_in port (index-aligned with overridable initializers)
    std::vector<std::optional<T>> _latchedConfig;
...
private:
    // DataSet<T> -> DataSet<T> ([1xN] single shot or [MxN] history)
    ...
    // precedence per initializer: fed config_in port > model_overrides entry > model-baked
    // default; a map entry that cannot be applied is reported via handleError instead of
    ...
    // run the session: output 0 is returned, outputs 1..n-1 go to the meta_out spans
    // (runAll only when a meta port is connected, so ORT can prune otherwise)
    [[nodiscard]] std::expected<std::vector<float>, gr::Error> runSession(std::span<const float> normalised, std::span<const NamedTensor> extras, bool wantMeta, auto& metaOuts, std::vector<std::size_t>& metaCounts) {
        if (!wantMeta) {
    ...
    void loadModel() {
        ...
        model_io_shape.value = _session.template modelIoShape<gr::Size_t>();
    ...
    // shared: resample → normalise → inference pipeline; a failed inference reports through
    // handleError (ErrorPolicy::Stop/Passthrough) and returns nullopt so callers pass through
