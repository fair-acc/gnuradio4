...
for _v in ("OMP_NUM_THREADS", "OPENBLAS_NUM_THREADS", "MKL_NUM_THREADS", "TF_NUM_INTEROP_THREADS", "TF_NUM_INTRAOP_THREADS"):
...
def convert_to_onnx(model: keras.Model) -> tuple[onnx.ModelProto, dict[str, str]]:
    ...
    kept = [o for o in onnx_model.opset_import if o.domain == "" or o.domain in used_domains]
    del onnx_model.opset_import[:]
    onnx_model.opset_import.extend(kept)

    metadata = {"input_size": str(N), "normalise_mode": "None", "architecture": "identity-tf", **provenance()}
