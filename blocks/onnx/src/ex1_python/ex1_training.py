...
class TrainConfig:
    ...
    seed: int = 42
    peak_width_min_bins: float = 1.0  # single-bin peaks allowed
    peak_width_max_frac: float = 0.15
    baseline_slope_max: float = 0.3
    lambda_heatmap: float = 1.0
    lambda_reg: float = 2.0
    num_workers: int = 8
    n_regression_channels: int = 8
    edge_margin: float = 0.05
    narrow_peak_prob: float = 0.3
    min_epochs: int = 30
    heatmap_sigma_min: float = 1.5  # minimum heatmap sigma in bins
    heatmap_sigma_scale: float = 0.5  # sigma = max(min, scale * sigma_equiv)
...
def generate_heatmap_target(peaks: list[PeakParams], cfg: TrainConfig) -> np.ndarray:
    ...
    for p in peaks:
        sigma = max(cfg.heatmap_sigma_min, cfg.heatmap_sigma_scale * p.sigma_equiv)
        radius = int(5 * sigma) + 1
        lo = max(0, int(p.center) - radius)
        hi = min(n, int(p.center) + radius + 1)
        x = np.arange(lo, hi)
        gaussian = np.exp(-0.5 * ((x - p.center) / sigma) ** 2)
...
def generate_regression_target(peaks: list[PeakParams], cfg: TrainConfig) -> np.ndarray:
    ...
    reg_target = np.zeros((n, R), dtype=np.float32)
    reg_mask = np.zeros(n, dtype=np.float32)

    for p in peaks:
        idx = int(np.clip(round(p.center), 0, n - 1))
        offset = (p.center - idx) / n
        frame_max_amp = max(pp.amplitude for pp in peaks) if peaks else 1.0

        def safe_val(v, default=0.0):
            return float(v) if np.isfinite(v) else default

        reg_target[idx, 0] = safe_val(offset, 0.0)
        reg_target[idx, 1] = safe_val(p.amplitude / frame_max_amp, 0.5)
        reg_target[idx, 2] = safe_val(p.sigma_equiv / n, 0.01)
        reg_target[idx, 3] = safe_val(p.w68 / n, 0.01)
        reg_target[idx, 4] = safe_val(p.w96 / n, 0.02)
        reg_target[idx, 5] = safe_val(p.w99 / n, 0.03)
        reg_target[idx, 6] = np.clip(safe_val(p.kurtosis / 10.0, 0.3), 0, 1)
        reg_target[idx, 7] = np.clip(safe_val((p.excess_kurtosis + 5) / 10.0, 0.5), 0, 1)
        reg_mask[idx] = 1.0
...
def _generate_batch_heatmap(args: tuple) -> tuple:
    ...
    Y_hm = np.empty((count, cfg.fft_size), dtype=np.float32)
    Y_reg = np.empty((count, cfg.fft_size, cfg.n_regression_channels), dtype=np.float32)
    Y_mask = np.empty((count, cfg.fft_size), dtype=np.float32)
...
def generate_dataset(cfg: TrainConfig, num_samples: int, seed: int,
    ...
    n = cfg.fft_size
    R = cfg.n_regression_channels
    X = np.empty((num_samples, n), dtype=np.float32)
...
def build_model(cfg: TrainConfig) -> keras.Model:
    ...
    model = keras.Model(inputs=inp, outputs=[heatmap, regression], name="peak_detector_unet")
    return model
...
class CombinedLoss(keras.losses.Loss):
    def __init__(self, n_regression_channels=8, lambda_heatmap=1.0, lambda_reg=2.0, **kwargs):
        super().__init__(**kwargs)
        self.R = n_regression_channels
    ...
    def call(self, y_true, y_pred):
        ...
        hm_true = y_true[:, :N]
        reg_true = tf.reshape(y_true[:, N:N + N * R], (-1, N, R))
        mask = y_true[:, N + N * R:]

        loss_hm = self.focal(hm_true, y_pred[0])
        loss_reg = masked_regression_loss(reg_true, y_pred[1], mask)
...
def train(cfg: TrainConfig, device: str, parallel: bool = True) -> keras.Model:
    ...
    # Concatenate targets: [heatmap(N), regression(N*R), mask(N)]
    N = cfg.fft_size
    R = cfg.n_regression_channels
...
def extract_peaks_from_heatmap(heatmap: np.ndarray, regression: np.ndarray,
    ...
    candidates = [(heatmap[i], i) for i in range(n) if local_max[i] and heatmap[i] >= threshold]
    ...
    for conf, idx in candidates:
        if suppressed[idx] or len(peaks) >= cfg.kmax:
            break

        center = idx + regression[idx, 0] * n
...
def plot_spectrum_with_peaks(cfg: TrainConfig, model: keras.Model = None,
    ...
    if model is not None:
        ...
        ax3.fill_between(bins, 0, heatmap[0], alpha=0.5, color='orange')
        ax3.plot(bins, heatmap[0], 'r-', linewidth=1)
...
def export_to_onnx(model: keras.Model, cfg: TrainConfig, output_dir: str = "."):
    ...
    n = cfg.fft_size
    R = cfg.n_regression_channels
    base_name = f"peak_detector_N{n}"
    saved_model_dir = f"{output_dir}/{base_name}_saved"
    onnx_path = f"{output_dir}/{base_name}.onnx"

    # Wrapper: input (batch,1,N) → transpose → model → concat [heatmap(N), regression(N*R)]
    ...
    try:
        ...
        _add_onnx_metadata(onnx_path, cfg)
...
def validate_onnx(onnx_path: str, cfg: TrainConfig, n_samples: int = 10):
    ...
    for i in range(n_samples):
        spectrum, _, _ = generate_single_spectrum(cfg, rng)
        normalized = spectrum_to_normalized(spectrum)
        x = normalized[np.newaxis, np.newaxis, :].astype(np.float32)

        ort_output = session.run(None, {input_name: x})[0]
        ort_heatmap = ort_output[0, :n]
        ort_regression = ort_output[0, n:].reshape(n, R)

        diff = np.max(np.abs(ort_output))
        max_diff = max(max_diff, diff)
        print(f"  Sample {i}: heatmap max={ort_heatmap.max():.4f}, "
              f"regression max={np.abs(ort_regression).max():.4f}")

    print(f"\nValidation passed: ORT inference produces valid outputs")
...
def main():
    ...
    parser.add_argument("--train-samples", type=int, default=0,
                        help="Override training sample count (0 = use TrainConfig default)")
    parser.add_argument("--threshold", type=float, default=0.4)
    parser.add_argument("--validate-onnx", type=str, default=None,
                        help="Path to .onnx file: compare TF vs ORT inference and exit")
    ...
    if args.train_samples > 0:
        ...
        cfg.val_samples = max(1000, args.train_samples // 10)
        cfg.test_samples = max(1000, args.train_samples // 10)

    device, _ = print_device_info()
    print(f"\nConfig: N={cfg.fft_size}, Kmax={cfg.kmax}, SNR=[{cfg.snr_min_db}, {cfg.snr_max_db}] dB")

    if args.validate_onnx:
        validate_onnx(args.validate_onnx, cfg)
        return
