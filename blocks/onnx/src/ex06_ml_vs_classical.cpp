...
struct DetectionEvent {
    ...
    float         trueCentre            = 0.f;
    float         detectedCentre        = 0.f;
    float         trueSigma             = 0.f;
...
std::string getStringProp(const gr::property_map& props, const char* key) { return std::string(props.value_or<std::string_view>(key, std::string_view{})); }
    ...
    for (std::size_t specIdx = 0; specIdx < kNumSpectra; ++specIdx) {
        ...
        if (!output.timing_events.empty()) {
            for (const auto& [detIdx, detProps] : output.timing_events[0]) {
                // fractional centre keeps the sub-bin position accuracy; the event index is an integer bin
                float detCentre = getProp(detProps, "centre", static_cast<float>(detIdx));
                ...
                for (std::size_t gi = 0; gi < gtPeaks.size(); ++gi) {
                    ...
                    if (std::abs(detCentre - gp.centre) < matchRadius) {
                        gtMatched[gi] = true;
                        matched       = true;
...
void printStats(const LatencyStats& stats) {
    ...
    std::println("    Position MAE:       {:.2f} bins", stats.positionMae);
    std::println("    Width MAE:          {:.2f} bins", stats.widthMae);
}
...
int main(int argc, char* argv[]) {
    ...
    mlDetector.gate_threshold = 0.3f;
