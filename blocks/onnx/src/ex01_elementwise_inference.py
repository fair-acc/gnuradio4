...
def main() -> int:
    in_dir = Path(sys.argv[1]) if len(sys.argv) > 1 else Path(__file__).resolve().parent.parent / "models"
    model_path = in_dir / f"mvp_affine_N{N}.onnx"
    if not model_path.exists():
        print(f"model not found: {model_path} (run ex01_elementwise_gen.py first)", file=sys.stderr)
        return 1
    ...
    print(f"override a=3, b=0 (as model_overrides would set them): max |error| = {override_error:.2e}")
