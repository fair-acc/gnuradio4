...
peaks_fixture_N1024.onnx / peaks_fixture_M16_N1024.onnx (target "peaks") implement
...
Usage: python fixtures_gen.py [output_dir] [--only TARGET [TARGET ...]]
"""
...
def provenance(target: str) -> dict[str, str]:
    rev = subprocess.run(["git", "rev-parse", "--short", "HEAD"], capture_output=True, text=True, cwd=Path(__file__).parent)
    return {
        "generator_cmd": f"python blocks/onnx/src/fixtures_gen.py --only {target}",
        "git_rev": rev.stdout.strip() if rev.returncode == 0 else "unknown",
        "created": datetime.now(timezone.utc).strftime("%Y-%m-%d"),
    }


def build_peaks_model(m: int = 1) -> onnx.ModelProto:
    initializers = [
        ...
        const("gate_K", np.full((1, K), 0.5, dtype=np.float32)),  # in-graph keep gate for residual
    ...
    if m > 1:
        ...
        input_nodes = [
            helper.make_node("Slice", ["spectra", "slice_m1", "slice_m", "axes_1"], ["newest"]),
    ...
    nodes = [
        *input_nodes,
        ...
        helper.make_node("TopK", ["masked", "k_topk"], ["score", "idx"], axis=1, largest=1, sorted=1),
        ...
        *[
            ...
            for node in [
                ...
                helper.make_node("Clip", [f"idx_m_{tag}_raw", "zero_i", "nmax_i"], [f"idx_m_{tag}"]),
                helper.make_node("Clip", [f"idx_p_{tag}_raw", "zero_i", "nmax_i"], [f"idx_p_{tag}"]),
                helper.make_node("GatherElements", ["x", f"idx_m_{tag}"], [f"yl_{tag}"], axis=1),
                helper.make_node("GatherElements", ["x", f"idx_p_{tag}"], [f"yr_{tag}"], axis=1),
                ...
                helper.make_node("Div", [f"asum_{tag}", f"amp2_{tag}"], [f"ratio_{tag}_raw"]),
                helper.make_node("Clip", [f"ratio_{tag}_raw", "r_lo_f", "r_hi_f"], [f"ratio_{tag}"]),
        ...
        helper.make_node("Concat", ["score_3", "centre_3", "amp_3", "sigma_3", "sigma_3", "zeros_3", "sigma_3", "score_3", "zeros_3", "is_top1_3", "zeros_3", "zeros_3", "zeros_3"], ["peaks"], axis=2),
        ...
        helper.make_node("ReduceSum", ["gauss_real", "axes_1"], ["recon_peaks"], keepdims=0),  # [1, N]
...
def self_test_peaks(path: Path, m: int = 1) -> None:
    ...
    for centre, amp, sig in [(200.5, 5.0, 5.0), (600.0, 2.5, 20.0)]:
    ...
    assert heatmap.shape == (1, N) and peaks.shape == (1, K, 13) and rescore.shape == (1, K)
    top, second = peaks[0, 0], peaks[0, 1]
    assert abs(top[1] - 200.5) < 1.0, f"centre {top[1]} != 200.5"
    assert abs(top[2] - 5.0) < 0.5, f"amplitude {top[2]} != 5.0"
    assert abs(top[3] - 5.0) < 0.5, f"sigma {top[3]} != 5.0"
    assert abs(second[1] - 600.0) < 1.5, f"centre {second[1]} != 600"
    assert abs(second[3] - 20.0) < 2.0, f"wide sigma {second[3]} != 20"
    scaled_rescore, scaled_heatmap = session.run(["peak_rescore", "heatmap"], {"spectra": frames, "score_scale": np.array([2.0], dtype=np.float32)})
    assert np.allclose(scaled_rescore, 2.0 * rescore, atol=1e-6), "score_scale override ignored on peak_rescore"
    assert np.allclose(scaled_heatmap, 2.0 * heatmap, atol=1e-6), "score_scale override ignored on heatmap"
    if m > 1:
        ...
        assert float(stale_scores.max()) < 0.5, f"stale-frame peak leaked: {stale_scores.max()}"
    print(f"self-test OK (M={m}): centre={top[1]:.2f} amplitude={top[2]:.2f} sigma={top[3]:.2f}")
...
def self_test_identity(path: Path, n: int, m: int) -> None:
    ...
    assert np.allclose(x, y, atol=1e-7), f"identity mismatch, max error {np.max(np.abs(x - y)):.2e}"
...
def build_affine_model() -> onnx.ModelProto:
    ...
    model = helper.make_model(graph, opset_imports=[helper.make_opsetid("", 17)], ir_version=8)
    metadata = {"input_size": str(n), "normalise_mode": "None", "architecture": "analytic-affine", **provenance("affine")}
    for key, value in metadata.items():
        entry = model.metadata_props.add()
        entry.key, entry.value = key, value
    onnx.checker.check_model(model)
...
def self_test_affine(path: Path) -> None:
    ...
    assert np.allclose(y, expected, atol=1e-6), f"max error {np.max(np.abs(y - expected)):.2e}"
    print(f"self-test OK (affine): y = {AFFINE_SCALE}*x + {AFFINE_OFFSET}, max error {np.max(np.abs(y - expected)):.2e}")


def build_frame_mean_model() -> onnx.ModelProto:
    n, m = N_SMALL, M_FRAME_MEAN
    nodes = [helper.make_node("ReduceMean", ["input"], ["output"], axes=[1], keepdims=0)]
    graph = helper.make_graph(
        nodes,
    ...
    model = helper.make_model(graph, opset_imports=[helper.make_opsetid("", 17)], ir_version=8)
    metadata = {"input_size": str(n), "history_depth": str(m), "normalise_mode": "None", "architecture": "analytic-frame-mean", **provenance("frame_mean")}
    for key, value in metadata.items():
        entry = model.metadata_props.add()
        entry.key, entry.value = key, value
    onnx.checker.check_model(model)
...
def self_test_frame_mean(path: Path) -> None:
    ...
    n, m = N_SMALL, M_FRAME_MEAN
    session = ort.InferenceSession(str(path), providers=["CPUExecutionProvider"])
    frames = np.stack([np.full(n, float(k), dtype=np.float32) for k in range(m)]).reshape(1, m, n)
    (y,) = session.run(None, {"input": frames})
    expected = np.full((1, n), (m - 1) / 2.0, dtype=np.float32)  # mean of 0..m-1
    assert y.shape == (1, n), f"output shape {y.shape} != (1, {n})"
    assert np.allclose(y, expected, atol=1e-6), f"max error {np.max(np.abs(y - expected)):.2e}"
    print(f"self-test OK (frame_mean): mean of frames 0..{m - 1} = {(m - 1) / 2.0}, max error {np.max(np.abs(y - expected)):.2e}")
...
def build_frame_delta_model() -> onnx.ModelProto:
    ...
    nodes = [
        helper.make_node("Slice", ["input", "slice_m1", "slice_m", "axes_1"], ["newest"]),
        helper.make_node("Slice", ["input", "slice_0", "slice_1", "axes_1"], ["oldest"]),
    ...
    model = helper.make_model(graph, opset_imports=[helper.make_opsetid("", 17)], ir_version=8)
    metadata = {"input_size": str(n), "history_depth": str(m), "normalise_mode": "None", "architecture": "analytic-frame-delta", **provenance("frame_delta")}
    for key, value in metadata.items():
        entry = model.metadata_props.add()
        entry.key, entry.value = key, value
    onnx.checker.check_model(model)
...
def self_test_frame_delta(path: Path) -> None:
    ...
    frames = np.stack([np.full(n, float(k), dtype=np.float32) for k in range(m)]).reshape(1, m, n)
    (y,) = session.run(None, {"input": frames})
    expected = np.full((1, n), float(m - 1), dtype=np.float32)  # newest (m-1) minus oldest (0)
    assert y.shape == (1, n), f"output shape {y.shape} != (1, {n})"
    assert np.allclose(y, expected, atol=1e-6), f"max error {np.max(np.abs(y - expected)):.2e}"
    (y_rev,) = session.run(None, {"input": frames[:, ::-1, :].copy()})
    assert np.allclose(y_rev, -expected, atol=1e-6), "reversed frame order must negate the delta"
    print(f"self-test OK (frame_delta): newest - oldest = {float(m - 1)}, order-sensitivity confirmed")
...
def build_mean_rms_model() -> onnx.ModelProto:
    ...
    model = helper.make_model(graph, opset_imports=[helper.make_opsetid("", 17)], ir_version=8)
    metadata = {"input_size": str(n), "normalise_mode": "None", "architecture": "analytic-mean-rms", **provenance("mean_rms")}
    for key, value in metadata.items():
        entry = model.metadata_props.add()
        entry.key, entry.value = key, value
    onnx.checker.check_model(model)
...
def self_test_mean_rms(path: Path) -> None:
    ...
    assert mean.shape == (1, 1) and rms.shape == (1, 1), f"shapes {mean.shape}, {rms.shape}"
    assert abs(mean.item() - np.mean(x)) < 1e-4, f"mean {mean.item()} != {np.mean(x)}"
    assert abs(rms.item() - np.sqrt(np.mean(x * x))) < 1e-3, f"rms {rms.item()} != {np.sqrt(np.mean(x * x))}"
    print(f"self-test OK (mean_rms): mean={mean.item():.4f} rms={rms.item():.4f}")


def convert_to_ort(onnx_path: Path) -> Path:
...
def generate_peaks(out_dir: Path) -> None:
    for m, name in [(1, f"peaks_fixture_N{N}.onnx"), (M_TEMPORAL, f"peaks_fixture_M{M_TEMPORAL}_N{N}.onnx")]:
        path = out_dir / name
        onnx.save(build_peaks_model(m), str(path))
        print(f"wrote {path} ({path.stat().st_size} bytes)")
        self_test_peaks(path, m)
...
def main() -> int:
    ...
    parser.add_argument("--only", nargs="+", choices=sorted(FIXTURES), metavar="TARGET", help=f"subset of {{{', '.join(sorted(FIXTURES))}}} (default: all)")
    args = parser.parse_args()
    args.out_dir.mkdir(parents=True, exist_ok=True)
    for target in args.only or sorted(FIXTURES):
        FIXTURES[target](args.out_dir)
    return 0
