...
def self_test(onnx_path: Path, ort_path: Path) -> None:
    ...
    a_override, b_override = np.array([3.0], dtype=np.float32), np.array([0.0], dtype=np.float32)
    ...
    assert override_error < 1e-6, f"overridden coefficients: max error {override_error:.2e}"
    assert not np.allclose(y, y_override), "override had no effect on the output"

    ort_session = ort.InferenceSession(str(ort_path), session_options, providers=["CPUExecutionProvider"])
    (y_ort,) = ort_session.run(None, {"x": x})
    (y_ort_override,) = ort_session.run(None, {"x": x, "a": a_override, "b": b_override})
    assert np.allclose(y_ort, y), ".ort default output diverges from .onnx"
    assert np.allclose(y_ort_override, y_override), ".ort override diverges from .onnx (initializer got baked in?)"

    print(f"self-test OK: y = {A_DEFAULT}*x + {B_DEFAULT} (max error {max_error:.2e}); " f"override a=3,b=0 verified live in both .onnx and .ort (max error {override_error:.2e})")


def main() -> int:
    out_dir = Path(sys.argv[1]) if len(sys.argv) > 1 else Path(__file__).resolve().parent.parent / "models"
