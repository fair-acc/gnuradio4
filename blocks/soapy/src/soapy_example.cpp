...
gr::Graph createGraph(std::string fileName1, std::string fileName2, gr::Size_t maxFileSize, float sampleRate, double rxCenterFrequency, double bandwidth, double rxGains) {
    ...
    if (fileName1.contains("null")) {
        ...
        expect(eq(gr::ConnectionResult::SUCCESS, flow.connect2(source, source.out[0], fileSink1, fileSink1.in))) << "error connecting NullSink1";
    } else {
        ...
        expect(eq(gr::ConnectionResult::SUCCESS, flow.connect2(source, source.out[0], fileSink1, fileSink1.in))) << "error connecting BasicFileSink1";
    }
    ...
    if (fileName2.contains("null")) {
        ...
        expect(eq(gr::ConnectionResult::SUCCESS, flow.connect2(source, source.out[0], fileSink2, fileSink2.in))) << "error connecting NullSink2";
    } else {
        ...
        expect(eq(gr::ConnectionResult::SUCCESS, flow.connect2(source, source.out[0], fileSink2, fileSink2.in))) << "error connecting BasicFileSink2";
    }
