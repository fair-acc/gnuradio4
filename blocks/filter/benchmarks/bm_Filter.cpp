...
int main() {
    ...
    { // where the transform overtakes the tap, on the host
        ...
        tapSweepRow("fir_filter (time)", [](std::size_t nTaps) {
            ...
            return megaSamplesPerSecond<gr::filter::fir_filter<float, gr::filter::IRForm::TIME_DOMAIN>>("host", [nTaps](auto& dut) { dut.b = gr::Tensor<float>(lowPassTaps(nTaps)); }, budget.samples, budget.attempts);
        });
        tapSweepRow("fir_filter (frequency)", [](std::size_t nTaps) {
            const Budget budget = budgetFor(nTaps, false);
            return megaSamplesPerSecond<gr::filter::fir_filter<float, gr::filter::IRForm::FREQUENCY_DOMAIN>>(
                "host",
                [nTaps](auto& dut) {
                    dut.b                 = gr::Tensor<float>(lowPassTaps(nTaps));
                    dut.outputs_per_frame = frameOutputsFor(nTaps);
        ...
        tapSweepRow("BasicFilter FIR (time)", [](std::size_t nTaps) {
            ...
            return megaSamplesPerSecond<gr::filter::BasicFilter<float>>(
                ...
                [nTaps](auto& dut) {
                    ...
                    dut.filter_domain      = gr::filter::FilterDomain::Time;
        ...
        tapSweepRow("BasicFilter FIR (frequency)", [](std::size_t nTaps) {
            ...
            return megaSamplesPerSecond<gr::filter::BasicFilter<float>>(
                ...
                [nTaps](auto& dut) {
                    dut.filter_type        = gr::filter::FilterType::FIR;
                    dut.filter_domain      = gr::filter::FilterDomain::Frequency;
                    dut.coefficient_source = gr::filter::CoefficientSource::Manual;
                    dut.a                  = gr::Tensor<float>(std::vector<float>{1.0f}); // feed-forward only
                    dut.b                  = gr::Tensor<float>(lowPassTaps(nTaps));
                    dut.outputs_per_frame  = frameOutputsFor(nTaps);
    ...
    { // where the two forms cross, finely enough to choose a default from
        ...
        for (std::string_view domain : servedDomains()) {
            crossRow.template operator()<gr::filter::fir_filter<float, gr::filter::IRForm::TIME_DOMAIN>>("tap per sample", domain, true, //
                [](auto& dut, std::size_t nTaps) { dut.b = gr::Tensor<float>(lowPassTaps(nTaps)); });
            crossRow.template operator()<gr::filter::fir_filter<float, gr::filter::IRForm::FREQUENCY_DOMAIN>>("transform per frame", domain, false, [](auto& dut, std::size_t nTaps) {
                dut.b                 = gr::Tensor<float>(lowPassTaps(nTaps));
                dut.outputs_per_frame = frameOutputsFor(nTaps);
            });
    ...
    { // which of them can leave the host, and what it buys
        ...
        for (std::string_view domain : servedDomains()) {
            domainRow.template operator()<gr::filter::fir_filter<float, gr::filter::IRForm::TIME_DOMAIN>>("fir_filter (time)", domain, true, always, //
                [](auto& dut, std::size_t nTaps) { dut.b = gr::Tensor<float>(lowPassTaps(nTaps)); });
        }
        for (std::string_view domain : servedDomains()) {
            domainRow.template operator()<gr::filter::fir_filter<float, gr::filter::IRForm::FREQUENCY_DOMAIN>>("fir_filter (frequency)", domain, false, always, [](auto& dut, std::size_t nTaps) {
                dut.b                 = gr::Tensor<float>(lowPassTaps(nTaps));
                dut.outputs_per_frame = frameOutputsFor(nTaps);
            });
        }
