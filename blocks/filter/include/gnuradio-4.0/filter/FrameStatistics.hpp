...
/// one root-mean-square figure per frame of input. The shape shared by every per-frame statistic -- RMS, peak,
/// an AGC's gain estimate -- is many samples in, one figure out, repeated. Stated as a window, the framework
/// reduces each frame in its own work item, so a stream with many frames in flight parallelises without the
/// block knowing where it runs. Reducing a *single* large frame cooperatively is a different problem;
/// `gr::algorithm::Reduce` does that.
template<typename T>
...
struct RootMeanSquare : Block<RootMeanSquare<T>, Resampling<>, Stride<>> {
    ...
    [[nodiscard]] gr::work::Status processBulk(InputViewLike auto& input, OutputViewLike auto& output) const noexcept {
        ...
        for (std::size_t n = 0UZ; n < output.size(); ++n) {
            // accumulated wider than the samples: in float this reaches 0.7% error at the frame size `Limits`
            // already admits, and saturates to infinity two orders of magnitude inside float's own range
            double sumOfSquares = 0.;
            for (std::size_t i = 0UZ; i < frame; ++i) {
                const double sample = static_cast<double>(input[n * frame + i]);
                sumOfSquares += sample * sample;
            }
            output[n] = static_cast<T>(std::sqrt(sumOfSquares / static_cast<double>(frame)));
        }
