...
This block estimates the frequency of a signal using the algorithms described in:
  ...
  [1] Gaussian, GaussianDecimating
    ...
    Annotated<float, "n periods rate", Doc<"number of periods to average over">>      n_periods{4};
    Annotated<T, "epsilon", Doc<"numerical error threshold">>                         epsilon{T(1e-8)};

    GR_MAKE_REFLECTABLE(FrequencyEstimator, in, out, sample_rate, f_expected, f_min, f_max, n_periods, epsilon);
    ...
    HistoryBuffer<T>      _inputHistory{32};
    HistoryBuffer<T>      _outputHistory{32};

    gr::algorithm::FFT<T, std::complex<T>> _fftImpl{};
    std::vector<T>                         _inData            = std::vector<T>(1024UZ, T(0));
    std::vector<T>                         _window            = gr::algorithm::window::create<T>(gr::algorithm::window::Type::Nuttall, 1024UZ);
    std::vector<std::complex<T>>           _outData           = std::vector<std::complex<T>>(1UZ + 512 / 2UZ, T(0));
    std::vector<T>                         _magnitudeSpectrum = std::vector<T>(_outData.size(), 0);

    void settingsChanged(const property_map& /*old_settings*/, const property_map& new_settings) {
        if (new_settings.contains("Ns") || new_settings.contains("sample_rate") || new_settings.contains("f_expected") || new_settings.contains("f_min") || new_settings.contains("f_max")) {
    ...
    void initialiseFilter() {
        ...
        _inData.resize(_outputHistory.capacity(), T(0));
        _outData.resize(1UZ + _outputHistory.capacity() / 2UZ, std::complex<T>(T(0)));
        _window = gr::algorithm::window::create<T>(gr::algorithm::window::Type::Nuttall, _inData.capacity());
    }
...
private:
    ...
    T estimateFrequencyFFT() {
        ...
        _magnitudeSpectrum = computeMagnitudeSpectrum(_fftImpl.compute(_inData), _magnitudeSpectrum, ConfigMagnitude{.computeHalfSpectrum = true, .outputInDb = false});

        std::size_t i_min = static_cast<std::size_t>((f_min / sample_rate) * _magnitudeSpectrum.size() * 2);
        std::size_t i_max = static_cast<std::size_t>((f_max / sample_rate) * _magnitudeSpectrum.size() * 2);

        // ensure indices are within bounds
        i_min = std::clamp(i_min, std::size_t(1), _magnitudeSpectrum.size() - 1);
        i_max = std::clamp(i_max, std::size_t(1), _magnitudeSpectrum.size() - 1);
        ...
        // Extract the magnitudes at the peak and its immediate neighbours
        T S_km1 = _magnitudeSpectrum[k_max - 1];
        T S_k   = _magnitudeSpectrum[k_max];
        T S_kp1 = _magnitudeSpectrum[k_max + 1];

        // fmt::println("found peak at: {} of {} -> ({}, {}, {})", k_max,  _magnitudeSpectrum.size(), S_km1, S_k, S_kp1);

        // Gaussian interpolation according to equation (21)
        T delta_k = (std::log(S_kp1) - std::log(S_km1)) / (T(4) * (std::log(S_k) - T(2) * std::log(S_kp1 * S_km1)));

        // calculate the interpolated frequency
        T interpolated_freq_bin = static_cast<T>(k_max) + delta_k;
        T interpolated_freq     = interpolated_freq_bin * sample_rate / static_cast<T>(_magnitudeSpectrum.size() * 2);
