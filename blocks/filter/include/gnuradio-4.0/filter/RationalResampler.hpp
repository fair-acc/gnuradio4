...
struct RationalResampler : Block<RationalResampler<T>, Resampling<>, Stride<>> {
    ...
    using Description = Doc<R""(resamples by the rational factor `interpolation`/`decimation` using one polyphase low-pass.

Every output lands exactly on an arm, so no interpolation between arms is needed and the ratio is met exactly.
`PolyphaseArbitraryResampler` takes any fixed ratio, interpolating between arms; `DriftResampler` takes one that moves
 ...
 * R. E. Crochiere and L. R. Rabiner, "Interpolation and decimation of digital signals - a tutorial review",
   Proc. IEEE, vol. 69, no. 3, pp. 300-331, 1981.)"">;
    ...
    void settingsChanged(const property_map& /*oldSettings*/, const property_map& /*newSettings*/) {
        ...
        // at least two taps per phase, or `decompose` normalises each phase to a single unit tap and the bank
        // degenerates into a zero-order hold -- an unfiltered rate change, with the whole imaging spectrum and a
        // DC gain of one to hide it. Reachable at the default n_taps for any interpolation above it.
        const std::size_t nTaps      = std::max(static_cast<std::size_t>(n_taps), 2UZ * L);
        const auto        prototype  = Algorithm::designPrototype(nTaps, cutoff);
