...
/// resamples at an arbitrary, possibly drifting ratio. Where `RationalResampler` states a fixed window and
/// lets the framework run those windows concurrently, this one cannot: the number of outputs per input is not
/// fixed, so it owns its own accounting and stays sequential -- the trade for a ratio that may move while the
/// stream runs.
template<typename T>
