...
/// (window_size-1)/2 samples, `Causal` is past-only with zero latency but non-linear phase.
template<typename T>
struct SavitzkyGolayFilter : Block<SavitzkyGolayFilter<T>> {
    ...
    using Description = Doc<R""(local polynomial smoothing/differentiation filter for streaming scalar data.

Fits a polynomial of `poly_order` over a `window_size` window by least squares and evaluates it, or its
`deriv_order`-th derivative, at one point. `SavitzkyGolayDataSetFilter` below is the zero-phase form over a whole
`DataSet`; this one streams, so `alignment` decides between a centred window (linear phase, group delay) and a
causal one (no latency, non-linear phase).
...
struct SavitzkyGolayDataSetFilter : Block<SavitzkyGolayDataSetFilter<T>> {
    ...
    using Description = Doc<R""(zero-phase Savitzky-Golay filter over a `DataSet`'s `signal_values`.

The whole record is present, so the window is always centred and the result has no group delay -- which a streaming
filter cannot offer. `SavitzkyGolayFilter` above is the streaming form.
 ...
 * A. Savitzky and M. J. E. Golay, "Smoothing and differentiation of data by simplified least squares
   procedures", Analytical Chemistry, vol. 36, no. 8, pp. 1627-1639, 1964.)"">;
    ...
    Annotated<gr::Size_t, "polynomial order", Doc<"order of fitting polynomial">>                                                                                                      poly_order      = 4U;
    Annotated<gr::Size_t, "derivative order", Doc<"derivative order (0=smooth, 1=1st deriv, ...)">>                                                                                    deriv_order     = 0U;
    Annotated<algorithm::savitzky_golay::BoundaryPolicy, "boundary policy", Doc<"how a record's edges are extended: Reflect mirrors them, every other value repeats the edge sample">> boundary_policy = algorithm::savitzky_golay::BoundaryPolicy::Reflect;
