...
const boost::ut::suite FrequencyEstimatorTests = [] {
    ...
    "Frequency Estimator General Tests"_test = [] {
        ...
        constexpr gr::Size_t num_samples = 55;      // number of samples for generating test signal
        ...
        // test frequencies
        std::vector<float> testFrequencies = {49.9f,                                                //
        ...
        for (float trueFreq : testFrequencies) {
            ...
            std::size_t numSamples = 3 * num_samples; // Ensure enough samples
            estimator.reset();
        ...
        for (std::size_t i = 0UZ; i < frequencyEstimates.size(); i++) {
            fmt::println("{}, {}", frequencyTrue[i], frequencyEstimates[i]);
        }
    };
};
