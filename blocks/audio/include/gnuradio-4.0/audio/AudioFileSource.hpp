...
For streaming large WAV files without memory buffering, use `WavSource` instead.)"">;
    ...
    void start() {
        ...
        if (!readerExp.has_value()) {
            fail("AudioFileSource::start()", readerExp.error());
            return;
        }
        resetState();
    ...
    [[nodiscard]] gr::work::Status processBulk(gr::OutputSpanLike auto& outSpan) {
        ...
        if (_readerFinalSeen) {
            if (_readerError.has_value()) {
                fail("AudioFileSource::processBulk()", *_readerError);
                outSpan.publish(0U);
                return gr::work::Status::ERROR;
            }
            ...
            if (!_decoded) {
                if (_accumulatedBytes.empty()) {
                    fail("AudioFileSource::processBulk()", gr::Error("audio file is empty"));
                    outSpan.publish(0U);
                    return gr::work::Status::ERROR;
                }
                ...
                if (!decodedExp.has_value()) {
                    fail("AudioFileSource::processBulk()", decodedExp.error());
                    outSpan.publish(0U);
                    return gr::work::Status::ERROR;
                }
