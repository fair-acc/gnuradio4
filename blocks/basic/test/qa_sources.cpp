...
const boost::ut::suite TagTests = [] {
    ...
    auto clockSourceTest = []<bool useIoThreadPool>(bool verbose = false) {
        ...
        Graph                testGraph;
        auto&                src = testGraph.emplaceBlock<ClockSource<std::uint8_t, useIoThreadPool>>({{gr::tag::SAMPLE_RATE.shortKey(), gr::pmt::Value(sample_rate)}, {"n_samples_max", gr::pmt::Value(n_samples)}, {"name", gr::pmt::Value("ClockSource")}, {"verbose_console", gr::pmt::Value(verbose)}});
        src.tags                 = {
