...
const suite<"SchmittTrigger Block"> triggerTests = [] {
    ...
    skip / "SchmittTrigger"_test =
        [&enableVisualTests]<class Method> {
            ...
            expect(eq(ConnectionResult::SUCCESS, graph.connect2(funcGen, funcGen.out, schmittTrigger, schmittTrigger.in))) << "connect funcGen->schmittTrigger";
            expect(eq(ConnectionResult::SUCCESS, graph.connect2(schmittTrigger, schmittTrigger.out, tagSink, tagSink.in))) << "connect schmittTrigger->tagSink";
            std::thread uiLoop;
            if (enableVisualTests) {
                ...
                // connect UI blocks
                expect(eq(ConnectionResult::SUCCESS, graph.connect2(funcGen, funcGen.out, uiSink1, uiSink1.in))) << "connect funcGen->uiSink1";
                expect(eq(ConnectionResult::SUCCESS, graph.connect2(schmittTrigger, schmittTrigger.out, uiSink2, uiSink2.in))) << "connect schmittTrigger->uiSink2";
