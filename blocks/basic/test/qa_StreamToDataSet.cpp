...
const boost::ut::suite<"StreamToDataSet Block"> selectorTest = [] {
    ...
    auto runUIExample = [](std::uint32_t lengthSeconds, std::string filter, gr::Size_t preSamples = 0U, gr::Size_t postSamples = 0U, gr::Size_t maxSamples = 10000U) {
        ...
        expect(eq(ConnectionResult::SUCCESS, graph.connect<"out", "clk_in">(clockSrc, funcGen))) << "connect clockSrc->funcGen";
        expect(eq(ConnectionResult::SUCCESS, graph.connect<"out", "in">(funcGen, sink))) << "connect funcGen->sink";
        expect(eq(ConnectionResult::SUCCESS, graph.connect<"out", "in">(funcGen, uiSink))) << "connect funcGen->uiSink";
        ...
        expect(eq(gr::ConnectionResult::SUCCESS, graph.connect<"out", "in">(funcGen, streamFilter))) << "connect funcGen->streamFilter";
        expect(eq(gr::ConnectionResult::SUCCESS, graph.connect<"out", "in">(funcGen, dataSetFilter))) << "connect funcGen->dataSetFilter";

        auto& uiFilteredStreamSink = graph.emplaceBlock<testing::ImChartMonitor<float>>({{"name", "ImChartFilteredStream"}});
        expect(eq(ConnectionResult::SUCCESS, graph.connect<"out", "in">(streamFilter, uiFilteredStreamSink))) << "connect funcGen->uiFilteredStreamSink";
        auto& uiDataSetSink = graph.emplaceBlock<testing::ImChartMonitor<DataSet<float>>>({{"name", "ImChartDataSet"}});
        expect(eq(ConnectionResult::SUCCESS, graph.connect<"out", "in">(dataSetFilter, uiDataSetSink))) << "connect funcGen->uiDataSetSink";
...
const boost::ut::suite<"StreamToStream test"> streamToStreamTest = [] {
    ...
    auto runTestStream = [](gr::Size_t nSamples, std::string filter, gr::Size_t preSamples, gr::Size_t postSamples, const std::vector<float>& expectedValues, std::size_t nTags) {
        ...
        expect(eq(gr::ConnectionResult::SUCCESS, graph.connect<"out", "in">(tagSrc, filterStreamToStream)));
        expect(eq(gr::ConnectionResult::SUCCESS, graph.connect<"out", "in">(filterStreamToStream, streamSink)));
...
const boost::ut::suite<"StreamToDataSet test"> streamToDataSetTest = [] {
    ...
    auto runTestDataSet = [](gr::Size_t nSamples, std::string filter, gr::Size_t preSamples, gr::Size_t postSamples, const std::vector<std::vector<float>>& expectedValues, const std::vector<std::size_t>& nTags, gr::Size_t maxSamples = 100000U) {
        ...
        expect(eq(gr::ConnectionResult::SUCCESS, graph.connect<"out", "in">(tagSrc, filterStreamToDataSet)));
        expect(eq(gr::ConnectionResult::SUCCESS, graph.connect<"out", "in">(filterStreamToDataSet, dataSetSink)));
