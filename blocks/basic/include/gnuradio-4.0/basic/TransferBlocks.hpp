...
struct HostToDevice : gr::Block<HostToDevice<T>> {
    ...
    Annotated<gr::Size_t, "chunk size", Limits<8U, 16U * 1024U * 1024U>> chunk_size = 4096UZ;
    ...
    gr::work::Status processBulk(gr::device::DeviceContext& ctx, InputSpanLike auto& input, OutputSpanLike auto& output) {
        const auto available = std::min(input.size(), output.size());
        const auto n         = std::min(available, static_cast<std::size_t>(chunk_size));
        if (n == 0) {
            std::ignore = input.consume(0);
