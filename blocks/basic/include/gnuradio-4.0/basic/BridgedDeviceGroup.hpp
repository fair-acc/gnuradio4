#ifndef GNURADIO_BRIDGED_DEVICE_GROUP_HPP
#define GNURADIO_BRIDGED_DEVICE_GROUP_HPP
...
/// wraps `members` into a DeviceGroup, inserting a `HostToDevice`/`DeviceToHost` transfer at every boundary port
/// that no interior edge already claims, then exports the transfers' outer ports.
template<typename T>
[[nodiscard]] inline std::expected<gr::DeviceGroupHandle, Error> makeBridgedDeviceGroup(gr::Graph&& members, std::source_location location = std::source_location::current()) {
    ...
    return gr::makeDeviceGroup(std::move(members), {}, location);
}

} // namespace gr::basic

#endif // GNURADIO_BRIDGED_DEVICE_GROUP_HPP
