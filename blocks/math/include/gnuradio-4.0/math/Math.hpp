    ...
    [[nodiscard]] constexpr V processOne(const V& a) const noexcept {
        if constexpr (gr::meta::any_simd<V, T>) {
            if constexpr (std::same_as<op, std::multiplies<T>>) {
                ...
                return a - value;
            } else if constexpr (std::same_as<op, std::bit_and<T>>) {
                return a & value;
            } else if constexpr (std::same_as<op, std::bit_or<T>>) {
                return a | value;
            } else if constexpr (std::same_as<op, std::bit_xor<T>>) {
                return a ^ value;
...
GR_REGISTER_BLOCK("gr::blocks::math::Max", gr::blocks::math::MathOpMultiPortImpl, ([T], gr::blocks::math::max<[T]>), [ float, double ])
GR_REGISTER_BLOCK("gr::blocks::math::Min", gr::blocks::math::MathOpMultiPortImpl, ([T], gr::blocks::math::min<[T]>), [ float, double ])
...
struct abs_op {
    constexpr T operator()(const T& v) const {
        if constexpr (std::is_unsigned_v<T>) {
            ...
            using Promoted = std::conditional_t<(sizeof(long long) > sizeof(int) && sizeof(T) > sizeof(int)), long long, int>;
...
GR_REGISTER_BLOCK("gr::blocks::math::Log10", gr::blocks::math::Log10, ([T]), [ float, double ])

template<typename T>
requires std::is_floating_point_v<T>
struct Log10 : gr::Block<Log10<T>> {
    using Description = Doc<R""(@brief Compute \(n \cdot \log_{10}(|x|) + k\)
...
struct Integrate : gr::Block<Integrate<T>> {
    using Description = Doc<R""(@brief Integrate successive samples and decimate.
...
in : 1 2 3 4 5 6 7 8   →  out : 10 26
    ...
    Annotated<gr::Size_t, "decim", Visible, Doc<"decimation / integration length">, Limits<1UZ, (1UZ << 20)>> decim = 1UZ;
