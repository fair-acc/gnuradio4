 ...
 * Each id is unique to its concrete type (enforced by add() rejecting duplicates);
 * findAs<T>(id) requires T to declare 'static constexpr std::string_view kId' and
 * static_casts the result of find(id) when id == T::kId.
 * N.B. this a RTTI-free requirement for embedded use-cases.
 */
