...
struct TestSpectrumGenerator : gr::Block<TestSpectrumGenerator<T>, gr::Resampling<1LU, 1LU>> {
    ...
    [[nodiscard]] gr::DataSet<T> createSpectrum(std::size_t N) {
        ...
        const auto   fMin = static_cast<double>(centre_freq) - static_cast<double>(signal_bandwidth) * 0.5;
        const auto   fMax = static_cast<double>(centre_freq) + static_cast<double>(signal_bandwidth) * 0.5;
        ...
        ds.meta_information[0] = {
            ...
            {std::pmr::string("centre_frequency"), gr::pmt::Value(static_cast<float>(centre_freq))},
...
private:
    ...
    void addSchottkyPeak(std::span<T> bins, std::size_t N, double t) {
        ...
        const double centreBin     = (0.5 + shift) * static_cast<double>(N);

        for (std::size_t i = 0; i < N; ++i) {
            const double dist     = static_cast<double>(i) - centreBin;
    ...
    void addNarrowLine(std::span<T> bins, std::size_t N, double position, double amplitudeDb) {
        ...
        const double centreBin     = position * static_cast<double>(N);
        const double invTwoSigmaSq = 1.0 / (2.0 * sigma * sigma);
        const auto   iMin          = static_cast<std::size_t>(std::max(0.0, centreBin - 5.0 * sigma));
        const auto   iMax          = std::min(N, static_cast<std::size_t>(centreBin + 5.0 * sigma + 1.0));
