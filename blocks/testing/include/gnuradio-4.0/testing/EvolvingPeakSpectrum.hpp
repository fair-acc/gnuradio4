...
private:
    ...
    [[nodiscard]] gr::DataSet<T> createSpectrum() {
        ...
        for (const auto& peak : _activePeaks) {
            ...
            if (amp > T(1e-6)) {
                addPeak(spectrum, N, peak.centre, peak.sigma, amp, peak.shape, peak.asymmetry);
            }
            ...
            if (emitTag) {
                ...
                peakEvents.push_back({
                    ...
                    {std::pmr::string("centre"), gr::pmt::Value(static_cast<float>(peak.centre))},
        ...
        for (auto& pe : peakEvents) {
            const auto centreVal = pe.template value_or<float>("centre", 0.0f);
    ...
    void maybeSpawnPeak(std::size_t N) {
        ...
        const T   centre     = uniform(edgeMargin, static_cast<T>(N) - edgeMargin);
    ...
    void addPeak(std::span<T> spectrum, std::size_t N, T centre, T sigma, T amplitude, PeakShape shape, T asymmetry) {
        switch (shape) {
