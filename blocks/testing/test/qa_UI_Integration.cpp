...
const boost::ut::suite TagTests = [] {
    ...
    "FunctionGenerator + ClockSource FAIR test"_test = [] {
        ...
        Graph testGraph;
        auto& clockSrc = testGraph.emplaceBlock<gr::basic::ClockSource<std::uint8_t>>({{"sample_rate", gr::pmt::Value(sample_rate)}, {"n_samples_max", gr::pmt::Value(N)}, {"name", gr::pmt::Value("ClockSource")}, {"verbose_console", gr::pmt::Value(false)}});
        initClockSource(clockSrc);

        auto& funcGen = testGraph.emplaceBlock<FunctionGenerator<float>>({{"sample_rate", gr::pmt::Value(sample_rate)}, {"name", gr::pmt::Value("FunctionGenerator")}});
        initFunctionGenerator(funcGen);

        auto& sink = testGraph.emplaceBlock<TagSink<float, ProcessFunction::USE_PROCESS_ONE>>({{"name", gr::pmt::Value("SampleGeneratorSink")}});
        ...
        auto& uiSink = testGraph.emplaceBlock<testing::ImChartMonitor<float>>({{"name", gr::pmt::Value("BasicImChartSink")}});
        ...
        std::thread uiLoop([&uiSink]() {
            ...
            while (uiSink.draw({{"reset_view", gr::pmt::Value(true)}}) != work::Status::DONE) { // mocks UI update loop with 25 Hz repetition
    ...
    "FunctionGenerator + ClockSource FAIR test - Console Debug"_test = [] {
        ...
        auto& uiSink = testGraph.emplaceBlock<testing::ImChartMonitor<float, false>>({{"reset_view", gr::pmt::Value(true)}, {"plot_graph", gr::pmt::Value(true)}, {"plot_timing", gr::pmt::Value(true)}, {"timeout_ms", gr::pmt::Value(static_cast<gr::Size_t>(400))}});
