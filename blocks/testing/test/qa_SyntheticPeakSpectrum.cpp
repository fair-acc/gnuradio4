...
// SyntheticPeakSpectrum's generateBaseline()/addNoise() sequence (dc_feature_prob=0.5,
...
// The first draw (dc-feature presence check against dc_feature_prob) comes out >= 0.5 for
// seed 42, so no DC feature is added and the sequence is pure Gaussian noise.
...
const boost::ut::suite<"SyntheticPeakSpectrum"> syntheticPeakSpectrumTests = [] {
    ...
    "default construction reports the ex1_training.py-matching defaults"_test = [] {
        ...
        expect(approx(block.peak_width_max_sigma.value, 250.f, 1e-6f));
