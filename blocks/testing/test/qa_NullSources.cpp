...
const boost::ut::suite<"Null[..] and Testing Blocks"> nullSourcesTests = [] {
    ...
    "NullSource->CountingSink"_test = []<typename T>(const T&) {
        ...
        expect(eq(g.connect2(src, src.out, sink, sink.in), ConnectionResult::SUCCESS));
    ...
    "ConstantSource->Copy->CountingSink"_test = []<typename T>(const T&) {
        ...
        expect(eq(g.connect2(src, src.out, copy, copy.in), ConnectionResult::SUCCESS));
        expect(eq(g.connect2(copy, copy.out, sink, sink.in), ConnectionResult::SUCCESS));
    ...
    "CountingSource->HeadBlock->CountingSink"_test = []<typename T>(const T&) {
        ...
        expect(eq(g.connect2(src, src.out, head, head.in), ConnectionResult::SUCCESS));
        expect(eq(g.connect2(head, head.out, sink, sink.in), ConnectionResult::SUCCESS));
    ...
    "SimCompute(zero)->CountingSink"_test = []<typename T>(const T&) {
        ...
        expect(eq(g.connect2(src, src.out, sim, sim.in), ConnectionResult::SUCCESS));
        expect(eq(g.connect2(sim, sim.out, sink, sink.in), ConnectionResult::SUCCESS));
