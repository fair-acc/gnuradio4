...
const boost::ut::suite<"Fourier Transforms"> fftTests = [] {
    ...
    "FFT flow graph example"_test = [] {
        ...
        expect(eq(gr::ConnectionResult::SUCCESS, flow1.connect2(source1, source1.out, fftBlock, fftBlock.in)));
        Scheduler sched1;
        ;
        ...
        for (int i = 0; i < 2; i++) {
            ...
            expect(eq(gr::ConnectionResult::SUCCESS, flow2.connect2(source2, source2.out, fft2, fft2.in)));
            Scheduler sched2;
            ;
