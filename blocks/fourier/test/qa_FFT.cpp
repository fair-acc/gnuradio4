...
static_assert(!gr::device::HasDeviceProcessBulkSpans<gr::blocks::fft::FFT<double>, double, std::complex<double>>, //
    "FFT must not be taken by the span tier: only its processBulk(ctx, ...) hatch may reach a device");
...
const boost::ut::suite<"Fourier Transforms"> fftTests = [] {
    ...
    "FFT processBulk tests"_test = []<typename T>() {
        ...
        for (std::size_t i = 0UZ; i < N_mag; ++i) {
            ...
            // a folded (real-input) spectrum sums each bin with its negative-frequency twin and so doubles; DC and,
            // for even N, Nyquist are their own mirror image and take 1/N
            constexpr bool  folded    = !std::remove_cvref_t<decltype(fftBlock)>::computeFullSpectrum;
            const bool      hasTwin   = !folded || (i != 0UZ && !(fftBlock.fft_size % 2U == 0U && i == fftBlock.fft_size / 2U));
            const ValueType linearMag = std::hypot(re, im) * (hasTwin ? ValueType(2) : ValueType(1)) / static_cast<ValueType>(fftBlock.fft_size);
