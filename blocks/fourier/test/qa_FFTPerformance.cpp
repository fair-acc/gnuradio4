...
StreamSummary streamChainFor(bool interiorEdge, std::size_t fftSize, int runTimeSeconds) {
    ...
    const std::string secondDomain = interiorEdge ? "gpu:sycl" : "host:sycl";
...
int main(int argc, char* argv[]) {
    ...
    const auto          resolves  = [](std::string_view domain) { return gr::device::DeviceContextRegistry::instance().isServedExactly(domain); };
    ...
    // 3) what keeping the edge between two device blocks off the host is worth; compare A against B, not against
    //    section 2. The crossing arm puts the second FFT on a DIFFERENT device, so compute placement moves with the
    //    edge: the two arms are not a clean isolation of edge residency, and cannot be -- two blocks on one device
    //    always share one domain, so no edge between them can be made to cross.
    if (syclAvailable && resolves("gpu:sycl"sv) && resolves("host:sycl"sv)) {
