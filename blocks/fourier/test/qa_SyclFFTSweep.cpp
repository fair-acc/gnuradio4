...
int main() {
    ...
    if (!gr::device::hostSyclIsServed()) { // the SKIP above already covered "no backend at all"
        std::printf("FAIL: a build with a SYCL backend must serve 'host:sycl'\n");
        return 1;
    }
    ...
    for (const std::string_view candidate : {std::string_view{"gpu:sycl"}, std::string_view{"host:sycl"}}) {
        gr::device::DeviceContext* sched = registry.isServedExactly(candidate) ? registry.tryResolve(candidate) : nullptr;
