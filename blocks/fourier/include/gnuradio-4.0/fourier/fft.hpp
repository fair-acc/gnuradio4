...
struct FFT : gr::Block<FFT<T, U>, gr::Resampling<1UZ, 1UZ, false>, gr::Stride<>> { // 1:1, runtime chunk = fft_size (set in settingsChanged)
    using Description = Doc<R""(raw forward/inverse FFT, outputting std::complex<T> rather than a DataSet, with batch count auto-detected from input size and dispatch to CPU SimdFFT or GPU SYCL (Stockham) per the compute_domain setting; a DataSet-emitting spectrum mode is available as the FFT<T, DataSet<P>> partial specialisation below.)"">;
    ...
    gr::work::Status processBulkDevice(gr::device::DeviceContext& ctx, InputSpanLike auto& inSpan, OutputSpanLike auto& outSpan)
...
struct FFT<T, DataSet<P>> : gr::Block<FFT<T, DataSet<P>>, gr::Resampling<1UZ, 1UZ, false>> {
    using Description = Doc<R""(FFT spectrum mode: N:1 resampling to one windowed magnitude/phase DataSet<P> per fft_size input samples, real-to-complex T giving the half spectrum [DC, +fs/2] and complex-like T the full fftshifted spectrum [-fs/2, +fs/2), with device dispatch (processBulkDevice) covering only T == std::complex<float>.)"">;
    ...
    requires std::same_as<T, std::complex<float>> // gr::device::SyclFFT is a float-only, complex-only tier
    {
        ...
        gr::device::parallelFor(ctx, total, [complexDev, windowDev, N](std::size_t idx) { complexDev[idx] = gr::algorithm::fft::applyWindowOne(complexDev[idx], windowDev[idx % N]); });
        ...
        gr::device::parallelFor(ctx, total, [fftOut, magDev, phaseDev, N, outputInDb](std::size_t idx) {
    ...
    /// driven straight from `processBulkDevice` the coefficients are still host-side, where a kernel cannot follow
    ...
    void freeDeviceScratch() {
        if (_deviceCtx == nullptr || !_deviceComplex) {
            return;
        }
