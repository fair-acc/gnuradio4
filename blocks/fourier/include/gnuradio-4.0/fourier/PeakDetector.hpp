...
// estimateWidth measures half-maximum crossings, and the iterative stripping is tuned around
// subtracting that width. The half-maximum width the search measures is also what PeakResult
// reports, so the published quantity matches the ML detector without retuning the search.
...
inline void subtractPeak(std::vector<float>& spectrum, float centre, float amplitude, float sigmaL, float sigmaR, float excessKurtosis, SubtractionShape shape) {
    ...
    for (std::size_t i = lo; i < hi; ++i) {
        ...
        case SubtractionShape::Voigt: {
            const float gaussian           = std::exp(-0.5f * x * x / (sigma * sigma));
            const float lorentzian         = sigma * sigma / (sigma * sigma + x * x);
            const float lorentzianFraction = std::clamp(excessKurtosis / 3.f, 0.f, 1.f);
            value                          = amplitude * ((1.f - lorentzianFraction) * gaussian + lorentzianFraction * lorentzian);
            break;
        }
...
// threshold defaults mirror PeakDetector's settings -- see the rationale there
[[nodiscard]] inline std::vector<PeakResult> detectPeaksIterative(std::span<const float> spectrum, float noiseRejectionThreshold = 5.0f, float minAmplitude = 0.0f, float minIsolationFraction = 0.5f, std::size_t maxIterations = 10, float minProminence = 5.0f, std::size_t maxPeaks = 10, SubtractionShape shape = SubtractionShape::Auto) {
    ...
    auto [noiseFloor, inputNoiseSigma] = estimateMagnitudeNoise(spectrum);
    ...
    for (std::size_t iter = 0; iter < maxIterations && peaks.size() < maxPeaks; ++iter) {
        const float residualFloor    = estimateMagnitudeNoise(std::span<const float>(residual)).median;
        const float currentThreshold = residualFloor + noiseRejectionThreshold * inputNoiseSigma;
        ...
        if (bestProm < minProminence * inputNoiseSigma) {
            break;
        }
        ...
        // from the original spectrum, not the residual
        ...
        auto [posUnc, widthUnc, ampUnc] = estimateUncertainty(inputNoiseSigma, peakAmp, sigma);
        ...
        // the subtraction shape is fitted through excess kurtosis, which the peaks-tensor contract
        // maps as kurt = 5*eta; reusing that mapping keeps both detectors on one width definition
        const ContainmentWidths classicalWidths = containmentWidths(halfLeft * kGaussianHalfMaxToSigma, halfRight * kGaussianHalfMaxToSigma, excessKurt / 5.f);
        peaks.push_back({
            .confidence           = bestProm / (inputNoiseSigma + 1e-10f),
            .centre               = centre,
            .amplitude            = peakAmp,
            .amplitudeMeasured    = ampMeas,
            .hwhmLeft             = halfLeft,
            .hwhmRight            = halfRight,
            .w68                  = classicalWidths.w68,
            .w96                  = classicalWidths.w96,
            .w99                  = classicalWidths.w99,
            .kurtosis             = excessKurt,
            .prominence           = bestProm / (inputNoiseSigma + 1e-10f),
            .isolation            = iso,
...
struct PeakDetector : gr::Block<PeakDetector> {
    ...
    // 5 is the lowest threshold with zero false positives on SyntheticPeakSpectrum's zero-peak
    // seeds; do not lower it without re-measuring there.
    ...
    Annotated<SubtractionShape, "subtraction shape"> subtraction_shape         = SubtractionShape::Auto;
    ...
    [[nodiscard]] gr::DataSet<float> processOne(gr::DataSet<float> inData) {
        ...
        if (n == 0UZ) { // more declared signals than values: the prominence loop below would wrap
            return inData;
        }
        std::span<const float> spectrum(inData.signal_values.data(), n);

        const SubtractionShape shape = subtraction_shape;
        ...
        auto [noiseFloor, noiseSigma] = estimateMagnitudeNoise(spectrum);
