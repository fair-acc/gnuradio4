...
static_assert(gr::refl::nttp_name<1u> == "1" or gr::refl::nttp_name<1u> == "1U" or gr::refl::nttp_name<1u> == "0x1");
...
int test0(Test& t) { return gr::refl::data_member<"foo">(t); }

auto test1() { return gr::refl::data_member_name<Test, 1>; }
...
static_assert(gr::refl::find_data_members<AndAnother, only_ints> == std::array<size_t, 4>{0, 1, 2, 6});
static_assert(gr::refl::find_data_members<AndAnother, name_is_out> == std::array<size_t, 1>{4});
static_assert(gr::refl::find_data_members_by_type<AndAnother, std::is_floating_point> == std::array<size_t, 2>{3, 4});
...
#define ARRAY
#endif
static_assert(std::same_as<gr::refl::data_member_types<AndAnother, ARRAY{0, 1}>, gr::meta::typelist<int, int>>);
static_assert(std::same_as<gr::refl::data_member_types<AndAnother, ARRAY{2, 5, 4}>, gr::meta::typelist<int, char, double>>);
static_assert(std::same_as<gr::refl::data_member_types<AndAnother>, gr::meta::typelist<int, int, int, float, double, char, int>>);
#undef ARRAY

static_assert([] {
    size_t sum = 0;
    gr::refl::for_each_data_member_index<AndAnother>([&sum](auto idx) { sum += idx; });
    return sum;
}() == 0 + 1 + 2 + 3 + 4 + 5 + 6);
...
const char* string_test() { return gr::refl::class_name<ns::Type<int>>.data(); }

std::string_view string_test2() { return gr::refl::class_name<ns::Type<float>>; }
