...
struct MergeView : std::ranges::view_interface<MergeView<R, TComp>> {
    ...
    using TSentinel = std::ranges::sentinel_t<TInView>;
    ...
    struct Iterator {
        ...
        explicit Iterator(MergeView* parent) : _view(parent) {
            ...
            for (auto&& v : _view->_out) {
                ...
                _ends.push_back(std::ranges::end(v));
            }
            next();
        }
        ...
        void next() {
            ...
            for (std::size_t i = 0; i < N; ++i) {
                ...
                if (best == N || std::invoke(_view->_comp, *_its[i], *_its[best])) {
                    best = i;
                }
            }
            _chosen = best;
