...
struct Options {
    ...
    std::string           registryInstance    = "gr::globalBlockRegistry(std::source_location::current())";
    std::string           registryDeclaration = "#include <source_location>\nnamespace gr { BlockRegistry& globalBlockRegistry(std::source_location location); }";

    bool        expansionsSplit = false; // true: each block instantiation creates its own file
    std::size_t expansionsBatch = 1UZ;   // instantiations per generated file when splitting
    bool        hooksOnly       = false; // emit only the registry hook, not the block bodies
    ...
    Options(int argc, char** argv) {
        ...
        for (int index = 3; index < argc; index++) {
            ...
            } else if (std::strcmp(argv[index], "--hooks-only") == 0) {
                hooksOnly = true;
            } else if (argc > index + 1 && (std::strcmp(argv[index], "--split-batch") == 0)) {
                expansionsBatch = std::max(1UL, std::strtoul(argv[index + 1], nullptr, 10));
            } else if (argc > index + 1 && (std::strcmp(argv[index], "--registry-header") == 0)) {
                registryHeader = argv[index + 1];
            } else if (argc > index + 1 && (std::strcmp(argv[index], "--registry-declaration") == 0)) {
                registryDeclaration = argv[index + 1];
...
struct GeneratedFiles {
    ...
    GeneratedFiles(std::string outFileBase, bool hooksOnly)
        : registrations(hooksOnly ? std::ofstream{} : openFile(outFileBase + ".cpp")), //
          declarations(outFileBase + "_declarations.hpp.in"),                          //
          rawCalls(outFileBase + "_raw_calls.hpp.in"),                                 //
          hook(openFile(outFileBase + "_hook.cpp")) {}
};

void emitHookCode(auto& hookOutput, std::string_view namePrefix, const Options& options) {
    hookOutput << std::format("// auto-generated registry hook, do not edit.\n");
    hookOutput << "#include <cstddef>\n\nnamespace gr { class BlockRegistry; }\n";
    // terminated here: a ';' in a CMake argument is a list separator and never reaches this tool
    hookOutput << std::format("{};\n", options.registryDeclaration);
    hookOutput << std::format("extern \"C\" std::size_t gr_blocklib_init_unit_{}(gr::BlockRegistry&);\n", namePrefix);
    hookOutput << std::format("[[maybe_unused]] static auto gr_blocklib_init_unit_{0}_invoked = gr_blocklib_init_unit_{0}({1});\n", namePrefix, options.registryInstance);
}
...
int main(int argc, char** argv) try {
    ...
    while (std::getline(fin, line)) {
        ...
        if (auto combos = cartesianProduct(info.expansions); !options.expansionsSplit || combos.empty()) {
            ...
            GeneratedFiles output(outFileBase, options.hooksOnly);
            ...
            emitHookCode(output.hook, namePrefix, options);
            ...
            int localIdx = 0;
            for (std::size_t batchStart = 0UZ; batchStart < combos.size(); batchStart += options.expansionsBatch) {
                std::vector<std::string> generatedRegistrationFunctions;
                ...
                const std::size_t batchEnd = std::min(batchStart + options.expansionsBatch, combos.size());
                for (std::size_t comboIdx = batchStart; comboIdx < batchEnd; ++comboIdx) {
                    const auto  replaced = replacePlaceholders(std::string(info.paramPack), combos[comboIdx]);
                    std::string finalName;
                    if (!info.baseName.empty() && !replaced.empty()) {
                        finalName = std::format("{}<{}>", info.baseName, replaced);
                    } else {
