...
int main() {
    ...
    constexpr T sampling_rate = 1024.0;            // ~1KHz
    constexpr T N             = 8 * sampling_rate; // 8 seconds worth of samples
    ...
    exprtk::function_compositor<T> compositor(symbol_table);
    compositor.load_vectors(true);

    using function_t = typename exprtk::function_compositor<T>::function;
    compositor.add(function_t("dft_1d_real").var("N").expression(R"(
           for (var k := 0; k < N; k += 1) {
               var k_real := 0.0;
               var k_imag := 0.0;
    ...
    const std::string dft_program = R"(
