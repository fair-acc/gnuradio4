...
constexpr void simdReordering(SimdFFT<T, transform, N_>& setup, std::span<const T> input, std::span<T> output) {
    using V4                = vec<T, 4>;
    constexpr std::size_t L = V4::size();
    ...
    if constexpr (SimdFFT<T, transform, N_>::IsRealValued::value) {
        assert((SimdFFT<T, transform, N_>::canProcessSize(N, Order::Ordered))); // non-multiple of 32 (while they can be computed) are very hard to bit-reverse
        if constexpr (direction == Direction::Forward) {
            const V4* vin  = reinterpret_cast<const V4*>(input.data());
            V4*       vout = reinterpret_cast<V4*>(output.data());

            const std::size_t dk = N / 32; // For N=48: dk=1

            for (std::size_t k = 0; k < dk; ++k) {
                V4 out0_0, out0_1, out2_0, out2_1;
            ...
            reversed_copy(dk, vin + 2, 8, reinterpret_cast<V4*>(output.data() + N / 2));

            // reversed_copy(dk, vin+6, 8, (v4sf*)(out + N))
            reversed_copy(dk, vin + 6, 8, reinterpret_cast<V4*>(output.data() + N));
        } else { // Backward
            const V4* vin  = reinterpret_cast<const V4*>(input.data());
            V4*       vout = reinterpret_cast<V4*>(output.data());
            ...
            for (std::size_t k = 0; k < dk; ++k) {
                V4 out0_0, out0_1, out4_0, out4_1;
            ...
            unreversed_copy(dk, reinterpret_cast<const V4*>(input.data() + N / 4), reinterpret_cast<V4*>(output.data() + N - 6 * L), -8);
            unreversed_copy(dk, reinterpret_cast<const V4*>(input.data() + 3 * N / 4), reinterpret_cast<V4*>(output.data() + N - 2 * L), -8);
        }
        return;
    }
    ...
    if constexpr (direction == Direction::Forward) {
        for (std::size_t k = 0; k < Ncvec; ++k) {
            ...
            V4                lo{}, hi{};
            interleave(V4(inP + (2 * k + 0) * L, stdx::vector_aligned), V4(inP + (2 * k + 1) * L, stdx::vector_aligned), lo, hi);
        ...
        for (std::size_t k = 0; k < Ncvec; ++k) {
            const std::size_t kk = (k / 4) + (k % 4) * (Ncvec / 4);
            V4                re{}, im{};
            uninterleave(V4(inP + (2 * kk + 0) * L, stdx::vector_aligned), V4(inP + (2 * kk + 1) * L, stdx::vector_aligned), re, im);
...
constexpr void transformInternal(SimdFFT<T, transform, N_>& setup, std::span<const T> inputSpan, std::span<T> outputSpan, std::span<T> scratch) {
    using V = SimdFFT<T, transform, N_>::vector_type;
    ...
    const int     numStages_odd = setup._radixPlan[1] & 1;
    ...
    if constexpr (direction == Direction::Forward) {
        ...
        if constexpr (SimdFFT<T, transform, N_>::IsRealValued::value) {
            ...
            if constexpr (ordering == Order::Ordered) {
                ...
                simdReordering<Direction::Forward>(setup, std::span<const T>{buff[!ib]}, buff[ib]);
            ...
            std::span<T> outp = cfftf1_ps<Direction::Backward, T>(n_vecs_cplx, buff[ib], buff[!ib], buff[ib], twiddle_span, setup._radixPlan);
            ib                = (outp.data() == buff[0].data()) ? 0 : 1;
        ...
        if constexpr (ordering == Order::Ordered) {
            simdReordering<Direction::Backward>(setup, inputSpan, buff[ib]);
            inputSpan = buff[ib];
            ib        = !ib;
        }
        ...
        if constexpr (SimdFFT<T, transform, N_>::IsRealValued::value) {
            ...
            std::span<T> outp = cfftf1_ps<Direction::Forward, T>(n_vecs_cplx, buff[ib], buff[0], buff[1], twiddle_span, setup._radixPlan);
            ib                = (outp.data() == buff[0].data()) ? 0 : 1;
