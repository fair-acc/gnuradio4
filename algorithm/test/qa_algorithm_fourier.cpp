...
const boost::ut::suite<"FFT common kernel-callable cores"> fftCommonCoreTests = [] {
    ...
    "magnitude core matches computeMagnitudeSpectrum wrapper for a half spectrum"_test = [] {
        ...
        for (std::size_t i = 0; i < coreOut.size(); ++i) {
            coreOut[i] = fft::computeMagnitudeOne(fftIn[i], fftIn.size(), false, i != 0UZ); // DC has no mirror twin to fold
        }
        expect(std::ranges::equal(wrapperOut, coreOut)) << "half-spectrum magnitude core vs. wrapper";

        expect(approx(wrapperOut[0], std::hypot(fftIn[0].real(), fftIn[0].imag()) / static_cast<double>(fftIn.size()), 1e-12)) << "DC is its own mirror image: 1/N, not 2/N";
        expect(approx(wrapperOut[1], 2. * std::hypot(fftIn[1].real(), fftIn[1].imag()) / static_cast<double>(fftIn.size()), 1e-12)) << "every bin with a twin still folds to 2/N";
    };
    ...
    "includeNyquist extends the half spectrum by one bin (rfft convention: DC..Nyquist inclusive)"_test = [] {
        ...
        // Nyquist, like DC, is its own mirror image for even N: it takes 1/N where the folded bins take 2/N
        expect(approx(magWithNyquist.back(), fft::computeMagnitudeOne(fftIn[fftIn.size() / 2UZ], fftIn.size(), false, false), 1e-12)) << "the extra bin is exactly the Nyquist bin";
        expect(approx(magWithNyquist.back(), std::hypot(fftIn[3].real(), fftIn[3].imag()) / static_cast<double>(fftIn.size()), 1e-12)) << "and it is not doubled";
