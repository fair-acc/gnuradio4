...
const boost::ut::suite<"PolyphaseResampler"> _polyphase = [] {
    ...
    "L/M conversion matches upsample-filter-decimate"_test = [] {
        for (const auto [interpolation, decimation] : std::vector<std::pair<std::size_t, std::size_t>>{{1UZ, 1UZ}, {3UZ, 2UZ}, {2UZ, 3UZ}, {5UZ, 4UZ}}) {
            ...
            for (std::size_t n = 0UZ; n < kOutputs; ++n) {
                const float got = Resampler::sampleAt<float>(window, phases, phaseLen, interpolation, decimation, n);
    ...
    "a unit prototype at 1:1 is a pure delay"_test = [] {
        ...
        for (std::size_t n = 0UZ; n < window.size(); ++n) {
            identity = identity && std::abs(Resampler::sampleAt<float>(window, phases, phaseLen, 1UZ, 1UZ, n) - window[n]) < 1e-6f;
        }
        expect(identity) << "one unit tap at 1:1 has nothing to do";
    };
};

const boost::ut::suite<"PolyphaseResampler complex"> _polyComplex = [] {
    using namespace boost::ut;
    using C = std::complex<float>;

    "real taps weight complex samples, each part independently"_test = [] {
        // a 1:1 unit tap must return the sample untouched, real and imaginary alike
        const std::vector<float> phases{1.f};
        std::vector<C>           window(8);
        for (std::size_t n = 0UZ; n < window.size(); ++n) {
            window[n] = C{static_cast<float>(n), -static_cast<float>(n)};
        }
        bool identity = true;
        for (std::size_t n = 0UZ; n < window.size(); ++n) {
            const C got = Resampler::sampleAt<C>(window, phases, 1UZ, 1UZ, 1UZ, n);
            identity    = identity && std::abs(got.real() - window[n].real()) < 1e-6f && std::abs(got.imag() - window[n].imag()) < 1e-6f;
        }
        expect(identity) << "a complex sample passes through a unit tap unchanged";
    };
};
