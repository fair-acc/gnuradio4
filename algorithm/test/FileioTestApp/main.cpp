...
void test_dialog_read() {
    std::println("dialog_read begin isMainThread:{}", isMainThread());
    ...
    std::thread readerThread{[reader] mutable {
        std::println("Dialog reader thread started, waiting for user to pick a file");

        bool        finished   = false;
        std::size_t totalBytes = 0;
        while (!finished) {
            reader.poll(
                [&](const fileio::Reader::PollResult& res) {
                    finished = res.isFinal;
                    if (res.data.has_value()) {
                        totalBytes += res.data.value().size();
                    } else {
                        std::println("Dialog error: {}", res.data.error().message);
                    }
                },
                std::numeric_limits<std::size_t>::max(), true);
        }
        std::println("Dialog read finished: {} bytes total, thread exiting", totalBytes);
    }};
    readerThread.detach();

    std::println("dialog_read end: Reader created, dialog triggered, thread detached.");
}
            ...
            reader.poll(
                [&](const fileio::Reader::PollResult& res) {
                    ...
                    if (res.data.has_value()) {
                        ...
                        std::println("Http get error: {}", res.data.error().message);
                    }
                },
                std::numeric_limits<std::size_t>::max(), true);
        }
        std::println("Http get read finished: {} bytes total, thread exiting", totalBytes);
    }};
    readerThread.detach();
...
void test_http_get_404() {
    ...
    std::thread readerThread{[reader] mutable {
        ...
        std::println("test_http_get_404 finished: {} bytes total, thread exiting", totalBytes);
    }};
    readerThread.detach();
