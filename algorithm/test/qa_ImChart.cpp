...
const boost::ut::suite<"ImChart"> windowTests = [] {
    ...
    "linear axis transform"_test = [](const double valueCoordinate) {
        ...
        expect(approx(valueCoordinate, static_cast<double>(identity(valueCoordinate)), 2.2)); // binning limited
    ...
    "optimal tick position"_test = [](std::size_t axisWidth) {
        ...
        "basic chart - log axis"_test = []() {
            ...
            constexpr double      xMin           = 0.1;
            constexpr double      xMax           = 1'000.0;
            ...
            for (auto subrange :
                 { sequence_range(0.1, 0.9, 0.1), sequence_range(1.0, 9.0, 1.0), sequence_range(10.0, 90.0, 10.0), sequence_range(100.0, 1000.0, 100.0) }) {
            ...
            auto chart = gr::graphs::ImChart<width, 16, LogAxisTransform>({ { xMin, xMax }, { -100, 0 } });
