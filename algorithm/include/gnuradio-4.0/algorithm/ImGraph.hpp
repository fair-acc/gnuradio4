...
inline LayoutPref getLayoutPref(const std::shared_ptr<gr::BlockModel>& block) {
    ...
    if (ui.contains("layout_pref")) {
        if (auto str = ui.value_or<std::string>("layout_pref", std::string{}); !str.empty()) {
            if (str == "horizontal") {
...
Point<T> getPosition(const std::shared_ptr<gr::BlockModel>& block) {
    ...
    if (ui.contains("pos_x")) {
        x = static_cast<T>(ui.find_value("pos_x").value_or(Value{}).value_or(static_cast<double>(Point<T>::invalid_position)));
    }
    if (ui.contains("pos_y")) {
        y = static_cast<T>(ui.find_value("pos_y").value_or(Value{}).value_or(static_cast<double>(Point<T>::invalid_position)));
    }
    return {x, y};
}
...
public:
    ...
    void syncFromUIConstraints() {
        ...
        Tensor<Value> vec = _readOrSeedPortVec(ui, mapKey, portCount);
        ...
        if (m.contains("side")) {
            const Value sideVal = m.find_value("side").value_or(Value{});
        ...
        if (m.contains("pos")) {
            gr::property_map tmp;
            tmp.insert_or_assign(std::string_view{"point"}, m.find_value("pos").value_or(Value{}));
        ...
        if (!posSet) {
            ...
            if (m.contains("pos_x")) {
                const Value posX = m.find_value("pos_x").value_or(Value{});
            ...
            if (m.contains("pos_y")) {
                const Value posY = m.find_value("pos_y").value_or(Value{});
        ...
        if (m.contains("direction")) {
            gr::property_map tmp;
            tmp.insert_or_assign(std::string_view{"direction"}, m.find_value("direction").value_or(Value{}));
            _exitDir = gr::utf8::fromPropertyMap(Direction{}, tmp);
        } else if (m.contains("exit_dir")) {
            const Value exitDirVal = m.find_value("exit_dir").value_or(Value{});
        ...
        if (!_style.isSet()) {
            if (m.contains("colour")) {
                if (auto str = m.value_or<std::string>("colour", std::string{}); !str.empty()) {
...
private:
    ...
    static Tensor<Value> _readOrSeedPortVec(gr::property_map& ui, const char* mapKey, std::size_t portCount) {
        bool seed = true;
        if (ui.contains(mapKey)) {
            const Value entryValue = ui.find_value(mapKey).value_or(Value{});
            seed                   = !entryValue.holds<Tensor<Value>>();
        }
        if (seed) {
            ui.insert_or_assign(std::string_view{mapKey}, Tensor<Value>(gr::extents_from, {portCount}));
        }
        const Value   entryValue = ui.find_value(mapKey).value_or(Value{});
        Tensor<Value> vec        = entryValue.value_or<Tensor<Value>>(Tensor<Value>{});
        if (vec.size() != portCount) {
            vec.resize({portCount}, Value{});
        }
        return vec;
    }
    ...
    void updateUIConstraints(const std::string& key, Value value) {
        if (!_block) {
            return;
        }
...
[[nodiscard]] inline EdgeType getEdgeType(const gr::Edge& edge) {
    ...
    if (ui.contains("edge_type")) {
        if (auto str = ui.value_or<std::string>("edge_type", std::string{}); !str.empty()) {
            if (str == "feedback") {
