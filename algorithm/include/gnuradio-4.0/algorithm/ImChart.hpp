...
#include <format> // TODO: remove for std::print until std::format and std::print is available in gcc & emscripten
#ifdef __GNUC__
...
inline void resetView() { std::println("\033[2J\033[H"); }
...
public:
    ...
    void draw(const TContainer1& xValues, const TContainer2& yValues, std::string_view datasetName = {}) {
        ...
        for (std::size_t bRowIdx = 0UZ; bRowIdx < arrayHeight; bRowIdx += kCellHeight) {
            for (std::size_t bColIdx = 0UZ; bColIdx < arrayWidth; bColIdx += kCellWidth) {
                ...
                switch (style) {
                case Style::Bars: screen.assign(std::format("{}{}{}", colourStr, kBars[dot], defaultColour)); break;
                case Style::Marker: screen.assign(std::format("{}{}{}", colourStr, kMarker[_n_datasets - 1], Color::get(Color::Type::Default))); break;
                case Style::Braille:
                default: screen.assign(std::format("{}{}{}", colourStr, kBrailleCharacter[dot], Color::get(Color::Type::Default))); break;
                }
            }
        }
    ...
    void reset() const noexcept { std::print("\033[0;0H"); }
    ...
    void printScreen() const noexcept {
        ...
        std::print("{}", Color::get(Color::Type::Default));
    }
