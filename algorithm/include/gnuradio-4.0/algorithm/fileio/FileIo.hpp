...
#if !defined(__EMSCRIPTEN__) && GR4_FILEIO_CPR_AVAILABLE
#include <cpr/cpr.h>
#endif
#if __EMSCRIPTEN__
...
struct ReaderState {
    ...
    ~ReaderState() {
#ifndef NDEBUG
        bool ok = finalPublished.load(std::memory_order_acquire);
        assert(ok && "Potential UB, ReaderState destroyed without final message (bug: caller dropped Reader too early)");
#endif
    }
};
...
struct WriterState {
    ...
    ~WriterState() {
#ifndef NDEBUG
        bool ok = done.load(std::memory_order_acquire);
        assert(ok && "WriterState destroyed before completion");
#endif
    }
...
#if !__EMSCRIPTEN__ && GR4_FILEIO_CPR_AVAILABLE
...
#endif // !__EMSCRIPTEN__ && GR4_FILEIO_CPR_AVAILABLE
...
[[nodiscard]] inline std::expected<Writer, gr::Error> writeAsync(std::string_view uri, std::span<const std::uint8_t> data, const WriterConfig& config = {}) {
    ...
    if (detail::isHttpUri(uri)) {
        if (config.mode != WriteMode::overwrite) {
            return std::unexpected(gr::Error{"writeAsync: append mode is not supported for HTTP(S) URIs"});
        }
    ...
    if (detail::isHttpUri(uri)) {
        ...
        return Writer{state};
#else
        return std::unexpected(gr::Error{"writeAsync: HTTP(S) disabled at build time"});
#endif
    } else if (detail::isFileUri(uri)) {
        const auto newPath = detail::stripFileUri(uri);
        if (!newPath.has_value()) {
