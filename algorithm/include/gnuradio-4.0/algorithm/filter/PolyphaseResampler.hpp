...
struct PolyphaseResampler {
    ...
    [[nodiscard]] static constexpr TSample sampleAt(std::span<const TSample> window, std::span<const T> phases, std::size_t phaseLen, std::size_t interpolation, std::size_t decimation, std::size_t n) noexcept {
        const std::size_t phase          = (n * decimation) % interpolation;
        const std::size_t newest         = (n * decimation) / interpolation + phaseLen - 1UZ;
        const std::size_t oldestInWindow = newest >= window.size() ? newest + 1UZ - window.size() : 0UZ;
        const T*          phaseTaps      = phases.data() + phase * phaseLen;
        TSample           acc{};
        if (oldestInWindow == 0UZ) {
            for (std::size_t j = 0UZ; j < phaseLen; ++j) {
                acc += phaseTaps[j] * window[newest - j];
            }
            return acc;
        }
        for (std::size_t j = oldestInWindow; j < phaseLen; ++j) {
            acc += phaseTaps[j] * window[newest - j];
        }
        return acc;
    }
