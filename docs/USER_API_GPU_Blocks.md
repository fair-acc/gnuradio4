...
## Style 2 — `processBulk(ctx, …)`: the expert extension point
...
```cpp
[[nodiscard]] gr::work::Status processBulk(gr::device::DeviceContext& ctx, InputSpanLike auto& in, OutputSpanLike auto& out) {
...
If your block declares a window with `Resampling<>`/`Stride<>` _and_ defines the context overload, the hatch wins:
...
span form (style 1s) runs as a **single** work item and is exempt from both; `processBulk(ctx, …)` runs on the host
...
- **No tags.** `publishTag` and `mergedInputTag()` are non-const, and a parallelised body is `const`, so the compiler
  already stops you. Emit tags from the span form or from `processBulk(ctx, …)`.
...
for. `qa_DeviceDspChain` counts device refusals and device barriers rather than classifying dispatches, so the share
of span-tier dispatches that avoid the check is not measured here; what it does pin is that a device-resident span takes no check at
...
A **group** is no exception: `makeDeviceGroup` gives the group block its members' domain, so an edge between two device
...
**`makeBridgedDeviceGroup` — when a whole group crosses together.** Hoists one shared `compute_domain` onto a set of
blocks so the group is placed as a unit rather than block by block (`blocks/basic/.../BridgedDeviceGroup.hpp`,
`qa_DeviceGroupVertical`). The group block carries that domain outward, so two such groups chain device-to-device with
...
Measured with `core/benchmarks/bm_DeviceDispatch`, sweeping the chunk one dispatch is handed, log-spaced from the
frame sizes filters actually run at up to the largest FFT the backend benchmarks use. 4 194 304 samples per row, best
of 5, six pinned cores. Three blocks bracket the shapes: a multiply is one flop per sample, a polynomial 128, and an
IIR biquad is strictly sequential.

The absolute figures below are from one machine -- an AMD Ryzen 9 5900X at 3.66 GHz with an NVIDIA GPU through
AdaptiveCpp, built Release with GCC 15 -- and are quoted to show where the crossovers sit, not as a specification.
Re-run the benchmark rather than trusting the numbers on different hardware.
...
the framework does. Below ~256 samples per dispatch a device loses on every block in the table -- by an order of
magnitude on the cheap ones (13x for the multiply, 28x for the IIR) and by ~4x on the arithmetic-bound polynomial.
...
to N=8_ — and the exact version of it is the barrier count asserted in `qa_DeviceGroupVertical`, which is
...
synchronisation: 2.3 ms per added stage is 16 dispatches, i.e. ~144 µs each, which is what that kernel costs on this CPU. The
