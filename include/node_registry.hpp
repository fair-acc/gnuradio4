...
using node_construction_params = tag_t::map_type;

class node_registry {
private:
    using node_type_handler = std::function<void(std::unique_ptr<fair::graph::node_model> &, const node_construction_params &)>;
    ...
    static auto
    create_handler() {
        return [](std::unique_ptr<fair::graph::node_model> &result, const node_construction_params &params) {
            using NodeType = NodeTemplate<ValueType>;

            if constexpr (std::is_constructible_v<NodeType, const node_construction_params &>) {
                ...
                fair::meta::print_types<fair::meta::message_type<"Can not default-construct the node instance, nor to construct it from const node_construction_params&">, NodeType>{};
            }
            return true;
        };
    }
...
public:
    ...
    create_node(std::string_view name, std::string_view type, const node_construction_params &params) {
