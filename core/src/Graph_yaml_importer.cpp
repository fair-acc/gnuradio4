...
void loadGraphFromMap(PluginLoader& loader, gr::Graph& resultGraph, gr::property_map yaml, std::source_location location) {
...
gr::property_map saveGraphToMap(PluginLoader& loader, const gr::Graph& rootGraph) {
...

    {
        ...
        gr::graph::forEachBlock<block::Category::All>(rootGraph, [&](const auto& block) {
            pmtv::map_t map;
            ...
            if (fullTypeName == "gr::Graph") {
                ...
                property_map graphYaml = saveGraphToMap(loader, subGraphDirect->blockRef());
                ...
                if (stored.contains("")) {
                    ...
                    const auto& settingsMap   = ctxParameters.back().settings; // write only the last parameters
...

    {
        ...
        gr::graph::forEachEdge<block::Category::All>(rootGraph, [&](const auto& edge) {
...
gr::Graph loadGrc(PluginLoader& loader, std::string_view yamlSrc, std::source_location location) {
    Graph      resultGraph;
    ...
    loadGraphFromMap(loader, resultGraph, *yaml, location);
    return resultGraph;
}

std::string saveGrc(PluginLoader& loader, const gr::Graph& rootGraph) { return pmtv::yaml::serialize(saveGraphToMap(loader, rootGraph)); }
} // namespace gr
