...
int main() {
    ...
    for (const std::string& domain : domains) {
        ...
        const double resident = domain == "gpu:sycl" ? bestOfSeconds([&domain, &deviceResident] { return runSplitterChain(domain, kSamples, 65536UZ, deviceResident); }) : boundary;
    ...
    crossOverSweep("multiply, 1 flop/sample (transfer-bound)", [](std::string_view domain, gr::Size_t n, std::size_t chunk) { return runParallelChain<Gain>(domain, n, chunk); });
    ...
    std::println("\n== cost per added stage in a chain of N device blocks, {} samples, {} KiB chunk ==", static_cast<std::size_t>(kChainSamples), kChainChunk / 1024UZ);
