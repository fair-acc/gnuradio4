...
[[maybe_unused]] inline const boost::ut::suite scheduler_tests = [] {
    ...
    auto cpu = std::make_shared<ThreadPoolWrapper>(std::make_unique<BasicThreadPool>(std::string(kDefaultCpuPoolId), TaskType::CPU_BOUND, 2U, 2U), TaskType::CPU_BOUND, std::string(kDefaultCpuPoolId), "CPU");
