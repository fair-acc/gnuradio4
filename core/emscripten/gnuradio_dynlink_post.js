...
function grMissingStub(symName) {
    return function grUnresolvedGot() { throw new Error('[gnuradio4 dynlink] call to unresolved GOT symbol: ' + symName); };
}
...
if (typeof reportUndefinedSymbols === 'function') {
    ...
    reportUndefinedSymbols               = function() {
        ...
        if (missing.length) {
            console.error('[gnuradio4 dynlink] unresolved GOT symbols (' + missing.length + '):', missing.slice(0, 30), missing.length > 30 ? ('... +' + (missing.length - 30) + ' more') : '');
        }
    };
}
})();
