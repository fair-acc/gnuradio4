...
struct SubGraph : gr::Block<SubGraph> {
    ...
    device::DeviceContext*                   _deviceContext = nullptr; // the group's single device domain, latched at start
    ...
    void startDispatch() {
        ...
        _deviceContext = nullptr; // cleared before the refusal below can return, or a restart keeps the last one
        ...
        for (const auto& member : _order) {
            ...
            if (outcome.refused) {
                gr::log::error("sub-graph '{}': member compute_domain '{}' is required and {}", this->name.value, outcome.declared, outcome.reason);
                for (auto& groupMember : _order) {
                    std::ignore = groupMember->changeStateTo(lifecycle::State::ERROR);
                }
                return;
            }
            if (outcome.context == nullptr) {
                gr::log::warning("sub-graph '{}': member compute_domain '{}' {} — functional fallback to 'host'; spell it '{}!' to make this a stop", this->name.value, outcome.declared, outcome.reason, outcome.declared);
                break;
            }
            _deviceContext = outcome.context;
            break;
        }
        if (_deviceContext != nullptr) {
            for (auto& member : _order) {
                member->setComputeBackend(*_deviceContext);
            }
        }
    ...
    [[nodiscard]] work::Result work(std::size_t requestedWork, [[maybe_unused]] device::DeviceContext& computeBackend = device::hostBackend()) noexcept {
        ...
        if (_deviceContext != nullptr) {
            if (auto deviceErr = _deviceContext->peekDeviceError()) {
                gr::log::error("sub-graph '{}': device fault in a member's kernel: {}", this->name.value, *deviceErr);
                return {requestedWork, performed, work::Status::ERROR};
            }
        }
...
[[nodiscard]] inline std::expected<SubGraphHandle, Error> makeSubGraph(gr::Graph&& members, const std::set<std::string>& doNotExport = {}, std::source_location location = std::source_location::current()) {
    ...
    handle.block  = std::static_pointer_cast<BlockModel>(std::make_shared<SubGraphWrapper>(std::move(wrapperSettings)));
