 ...
 * `PackedTensorElement` row of a `Tensor<Value>`. Both row types carry the type tag bytes
 * (`valueType`, `flags`) plus an `inlineValue` slot (8 B) AND a `payloadOffset` / `payloadLength`
 * pair pointing into the container's shared payload pool. The byte sequence is host/device/IPC-
 * portable (no host pointers) and round-trips verbatim through `ValueMap::from_blob` / `as_blob`.
 *
 * Per-`ValueType` encoding:
 *
 * @code
 *  ┌────────────────────────┬──────────────────────────────────────────────────────────┐
...
struct ValueView {
    ...
    [[nodiscard]] std::optional<T> get_if() const noexcept {
        ...
        // Defence-in-depth bound check on the rank/elementCount bytes. The trusted ingestion
        // path (`from_blob` -> `_validateTensorSubBlob`) already enforces these; the gate
        // exists for `Value::makeView`-injected bytes that bypass validation.
        const std::size_t extentsBytes = paddedTensorExtentsBytes(rank);
        if constexpr (gr::meta::kDebugBuild) {
            if (payloadByteCount() < kTensorBlobHeaderSize + extentsBytes) {
                return std::nullopt;
            }
        }
        ...
        if constexpr (gr::meta::kDebugBuild) {
            const auto headerBytes = kTensorBlobHeaderSize + extentsBytes;
            if (static_cast<std::uint64_t>(elementCount) * sizeof(ElemT) > payloadByteCount() - headerBytes) {
                return std::nullopt;
            }
        }
...
static_assert(sizeof(gr::pmt::Value) == 16UZ, "Value handle is std::byte* + std::pmr::memory_resource* (== 2 pointers)");
