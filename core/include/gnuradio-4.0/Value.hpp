...
alignas(kRecAlignment) inline constexpr std::array<std::byte, kRecMinSize> kMonostateRecord = [] {
    ...
    gr::wire::writeHeaderSized(record.data(), kRecMinSize, std::to_underlying(ValueType::Monostate), std::to_underlying(ContainerType::Scalar));
    return record;
}();
