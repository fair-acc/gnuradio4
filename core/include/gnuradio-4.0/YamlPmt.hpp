...
std::expected<T, ValueParseError> parseAs(std::string_view sv) {
    if constexpr (std::is_same_v<T, std::monostate>) {
        ...
        try {
            ...
            return std::unexpected(ValueParseError{0UZ, fmt::format("Invalid value for type of '{}' - error: {}", tempParse, e.what())});
        }
    } else if constexpr (std::is_integral_v<T>) {
        ...
        const std::vector<std::string_view> segments = split(trimmed, ",");
        if (segments.size() != 2) {
            return std::unexpected(ValueParseError{0UZ, "Invalid value for type"});
        }
