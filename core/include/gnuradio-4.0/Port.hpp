...
struct Port {
    ...
    struct InputSpan : public ReaderSpanType<spanReleasePolicy> {
        ...
        [[nodiscard]] auto tags() {
            return std::views::transform(rawTags, [this](const Tag& tag) -> PairRelIndexMapRef { return PairRelIndexMapRef{relIndex(tag.index, streamIndex), std::cref(tag.map)}; });
        }
        ...
        [[nodiscard]] auto tags(std::size_t untilLocalIndex) {
            ...
            // return prefixRange | std::views::transform([this](const Tag& tag) -> PairRelIndexMapRef { return {relIndex(tag.index, streamIndex), std::cref(tag.map)}; });
