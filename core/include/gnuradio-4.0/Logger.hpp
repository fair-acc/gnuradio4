...
/**
 * @brief Bounded, non-blocking, best-effort store of the most recent N log records.
 *
 * `publish()` never blocks and never allocates: when the backend is busy (a concurrent
 * `publish`/`snapshot`/`drain`/`clear` holds it) it returns `false` and counts the loss.
 * `droppedBefore` on a retained record is a lower bound of records known to have been lost
 * before it, covering both capacity overwrites and contention drops. `clear()` is likewise
 * best-effort and no-ops when busy.
 */
...
private:
    ...
    std::uint64_t                    _droppedContended{};
    bool                             _busy{};
