...
/**
 * Singleton registry mapping `compute_domain` strings to device contexts.
 *
...
public:
    ...
    /// this domain, rather than whatever its ladder falls to: `tryResolve` answers for the whole ladder, so asking
    /// it whether 'gpu:sycl' is served says yes on a machine that serves only the SYCL CPU device
    [[nodiscard]] bool isServedExactly(std::string_view computeDomain) {
        std::scoped_lock       lk(_mtx);
        const DomainResolution resolution = resolveUnlocked(computeDomain);
        return !resolution.downgraded && servedUnlocked(resolution.resolved) != nullptr;
    }
};
...
/**
 * Resolve a declared compute domain, applying the '!'-required and downgrade policy.
 *
