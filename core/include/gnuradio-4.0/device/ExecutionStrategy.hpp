...
auto canProcessBulkDeviceInvokeTest(DeviceContext& ctx, auto& block, auto& inputSpans, auto& outputSpans, std::index_sequence<InIdx...>, std::index_sequence<OutIdx...>) -> decltype(block.processBulkDevice(ctx, std::get<InIdx>(inputSpans)..., std::get<OutIdx>(outputSpans)...));
...
[[nodiscard]] gr::work::Status invokeProcessBulkDevice(DeviceContext& ctx, auto& block, auto& inputSpans, auto& outputSpans, std::index_sequence<InIdx...>, std::index_sequence<OutIdx...>) {
    return block.processBulkDevice(ctx, std::get<InIdx>(inputSpans)..., std::get<OutIdx>(outputSpans)...);
}
...
concept HasDeviceBulkHatch = requires(DeviceContext& ctx, TBlock& block, InputSpans& inputSpans, OutputSpans& outputSpans) {
    { detail::canProcessBulkDeviceInvokeTest(ctx, block, inputSpans, outputSpans, std::make_index_sequence<std::tuple_size_v<std::remove_cvref_t<InputSpans>>>(), std::make_index_sequence<std::tuple_size_v<std::remove_cvref_t<OutputSpans>>>()) } -> std::same_as<gr::work::Status>;
};
 ...
 * spans that owns its own `consume`/`publish` and tags; and `processBulkDevice(ctx, spans...)`, the expert hatch,
...
private:
    ...
    requires HasDeviceBulkHatch<TBlock, InputSpans, OutputSpans>
    {
        ...
        if (auto deviceErr = ctx.pollDeviceError()) {
            return fail(std::format("device fault during processBulkDevice: {}", *deviceErr));
        }
    ...
    static DispatchResult dispatchDeviceProcessBulk(TBlock& block, DeviceContext& ctx, InputSpans& inputSpans, OutputSpans& outputSpans, std::size_t nIn, std::size_t nOut) {
        ...
        const WindowGeometry geometry      = kHasViewBody ? windowGeometry(block, nIn, nOut) : WindowGeometry{};
        ...
        if (!bodyOwnsIO && !runsAsWindows && detail::firstSerialBulkWarning(block)) {
            gr::log::warning("device dispatch: processBulk runs as one work item over the whole span, consuming and publishing all {} samples; declare a window with Resampling<>/Stride<> to have them run in parallel, or use processBulkDevice to own the accounting", count);
        }
    ...
    [[nodiscard]] static std::optional<DispatchResult> runSpanBody(TBlock& block, DeviceContext& ctx, InputSpans& inputSpans, OutputSpans& outputSpans, TInPtrs inPtrs, TOutPtrs outPtrs, //
        ...
        if (std::ranges::any_of(std::span<const DeviceSpanAccounting>{res.outAcct, nOutputs}, [](const DeviceSpanAccounting& a) { return a.tagPublishAttempted; })) {
            return refuseDeviceDispatch("processBulk published a tag from a device kernel; a tag payload is not device-constructible (run this block on the host, or use processBulkDevice)");
        }
        return std::nullopt;
    }
