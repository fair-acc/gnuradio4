...
/**
 * Which blocks may be bit-copied into device memory and read by a kernel body.
 *
...
/**
 * May the tag-publish probe run this block on a bit-copy?
 *
 * Every relocatable block may. The copy aliases whatever the original points at, so a probed call writes through a
 * pmr member into the live block -- `mutatesItsOwnState` captures those bytes and puts them back, which is what keeps the
 * probe an observation. A member seated on a device resource is the one case it declines, because the host may not
 * read the bytes it would have to restore.
 */
template<typename TBlock>
...
/**
 * Bit-copy a block into device memory so a kernel body can read its settings.
 *
...
/**
 * Refresh the settings a kernel reads, without disturbing anything else the mirror holds.
 *
