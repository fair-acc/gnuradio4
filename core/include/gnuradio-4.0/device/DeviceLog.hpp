...
// a bound, not a preference: a kernel writes a fixed-size slot with no allocator, so the count is compile-time
...
/**
 * POD handle over host-owned slab storage — the only state a kernel captures.
 *
 * Each slot holds one packed `gr::pmt::ValueMap` record; a kernel claims one with a single atomic bump, never
 * allocating and never blocking. Host and device must never write the slab concurrently — cross-device atomics
 * are not coherent — so the host drains only after a synchronisation barrier.
 */
...
/**
 * compile-time-checked format string that also captures the call site.
 *
 * `__builtin_FILE()`/`__builtin_LINE()` rather than `std::source_location`: both reduce to a string literal and an
 * int, which are addressable in device code.
 */
...
/**
 * kernel-facing logger handle with the same call syntax as the host front end.
 *
 * @code
 * struct Kernel {
 ...
 * @endcode
 *
 * Arguments must be arithmetic, `bool` or `std::string_view` — a `const char*` would be a device pointer the host
 * cannot dereference. `drainDeviceLog()` renders the text host-side.
 */
struct DeviceLogger {
    DeviceLogSlab slab{};
...
// host-side, after the drain: a kernel cannot format at the call site, so the arguments arrive type-erased and
// are substituted one at a time instead of in one std::format call
...
[[nodiscard]] inline std::size_t drainDeviceLog(const DeviceLogSlab& slab, Backend& backend) {
    ...
    for (std::uint32_t i = 0U; i < claimed; ++i) {
        ...
        record.locationTruncated = storeBounded(record.location, record.locationLength, detail::readString(map, kKeyFile));
        record.textTruncated     = storeBounded(record.text, record.textLength, detail::renderMessage(map));
...
namespace gr::device {
/**
 * owns the device log slab in the device-shared memory of a `DeviceContext`.
 *
 * Allocation goes through the backend-neutral `DeviceContext::allocateShared<T>()`, so one type serves the CPU
 * fallback, SYCL and any future context. A failed allocation leaves `view()` invalid and device emitters drop.
 */
class DeviceLogSlabStorage {
    DeviceContext* _context{};
    DeviceBuffer   _slots{};
    DeviceBuffer   _counters{}; // [0]=cursor, [1]=dropped, [2]=truncated
...
public:
    ...
    DeviceLogSlabStorage(DeviceContext& context, std::size_t slotCount, std::uint32_t slotBytes = gr::log::kDeviceLogSlotBytes) : _context(&context) {
        ...
        // ask for the alignment the slab actually requires: `allocateShared<std::byte>` would forward
        // `alignof(std::byte)`, i.e. 1, and only glibc's 16-byte malloc floor made that work by accident
        _slots               = context.allocate(slotCount * slotBytes, gr::pmt::kBlobAlignment, Residency::shared);
        _counters            = context.allocateShared<std::uint64_t>(3UZ);
        std::byte* slotsPtr  = _slots.devicePointer<std::byte>();
        auto*      countsPtr = _counters.devicePointer<std::uint64_t>();
        if (slotsPtr == nullptr || countsPtr == nullptr || (reinterpret_cast<std::uintptr_t>(slotsPtr) & (gr::pmt::kBlobAlignment - 1UZ)) != 0UZ) {
            release();
            return;
        }
    ...
    [[nodiscard]] gr::log::DeviceLogSlab view() const noexcept {
        ...
        std::uint64_t* counters = _counters.devicePointer<std::uint64_t>();
        return gr::log::DeviceLogSlab{.slots = _slots.devicePointer<std::byte>(), .cursor = &counters[0], .dropped = &counters[1], .truncated = &counters[2], .slotCount = _slotCount, .slotBytes = _slotBytes};
    }
...
/**
 * device-capable `gr::log::Backend`: host records go to the host backend, kernels write
 ...
 * context.wait();
 * std::ignore = gr::log::flush();
 * @endcode
 *
 * Host and device must never write the slab concurrently — cross-device atomics are not coherent — hence the
 * synchronise-then-flush order above.
 */
