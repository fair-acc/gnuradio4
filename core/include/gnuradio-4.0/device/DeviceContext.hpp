...
struct DeviceContextCpu final : DeviceContext {
    ...
    // nothrow so every backend's allocate* shares one null-on-failure contract (exception-free; SYCL/GLSL USM
    // already returns nullptr) — the dispatch path checks the result rather than catching bad_alloc.
    [[nodiscard]] void* allocateDeviceRaw(std::size_t bytes, std::size_t) override { return ::operator new(bytes, std::align_val_t{kAlign}, std::nothrow); }
    [[nodiscard]] void* allocateHostRaw(std::size_t bytes, std::size_t) override { return ::operator new(bytes, std::align_val_t{kAlign}, std::nothrow); }
    [[nodiscard]] void* allocateSharedRaw(std::size_t bytes, std::size_t) override { return ::operator new(bytes, std::align_val_t{kAlign}, std::nothrow); }
