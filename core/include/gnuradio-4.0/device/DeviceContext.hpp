...
/**
 * A backend allocation plus the one thing a `void*` cannot carry: who may dereference it.
 *
...
/**
 * Abstract base for backend-agnostic device memory management and data transfer.
 *
...
struct DeviceContext {
    ...
    /**
     * Typed escape for a body that genuinely needs one backend's own handle.
     *
...
public:
    ...
    /// Is this device's memory somewhere the host is not?
    /// True for a discrete device, where a kernel touching host-resident memory pays a bus transfer per access
    /// and staging a frame into device memory is worth the two bulk copies. False for a CPU backend, where the
    /// same staging is a memcpy that buys nothing — so a block asks this rather than assuming.
    [[nodiscard]] virtual bool hasRemoteMemory() const noexcept { return false; }
...
/**
 * CPU-only DeviceContext: heap allocation, memcpy transfers, no GPU.
 *
