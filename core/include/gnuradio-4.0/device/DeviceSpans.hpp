...
/**
 * What a kernel-side span records for the host to apply once the kernel has finished.
 *
 ...
 * it does for a `processBulk(ctx, ...)` hatch. Lives in device-visible memory and is read by the host after the barrier.
 */
...
/**
 * A kernel-side `InputSpanLike`: the samples, the input tags, and somewhere to record the consume.
 *
...
/// A kernel-side `OutputSpanLike`: the samples, the tag slots, and somewhere to record the publish.
template<typename T>
