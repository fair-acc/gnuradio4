...
struct DeviceContextGLSL final : DeviceContext {
    ...
    void dispatch(GlslProgram program, void* inBuf, void* outBuf, std::size_t count, std::size_t workgroupSize) {
    ...
    [[nodiscard]] std::expected<GlslProgram, std::string> compileOrGetCached(std::string_view glsl) { return _gl->compileOrGetCached(glsl); }
...
struct DeviceContextGLSL final : DeviceContext {
    ...
    void                                                  wait() override {}
    void                                                  dispatch(GlslProgram, void*, void*, std::size_t, std::size_t) {}
    [[nodiscard]] std::expected<GlslProgram, std::string> compileOrGetCached(std::string_view) { return std::unexpected("GLSL compute is not available"); }
