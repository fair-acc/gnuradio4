...
/**
 * latch for AdaptiveCpp's process-global asynchronous error list.
 *
...
/**
 * SYCL backend for DeviceContext: USM allocation, queue-based transfer, parallel dispatch.
 *
...
struct DeviceContextSycl final : DeviceContext {
    ...
    /// the shared-USM resource serving this context's domain, handed in by the runtime that registered both, so
    /// that a second lookup cannot answer with a different object than the edge allocator already uses. A context
    /// built by anything else -- a test, a plugin, a user with their own queue -- falls back to this queue's own.
    ...
    [[nodiscard]] std::pmr::memory_resource* resource(Access access) noexcept override {
        ...
        case Access::Shared: return sharedResource != nullptr ? sharedResource : &usmResourceFor(*queue);
        }
#else
        std::ignore = access;
#endif
    ...
    [[nodiscard]] bool hasRemoteMemory() const noexcept override {
#if GR_DEVICE_HAS_SYCL
        return queue != nullptr && queue->get_device().is_gpu();
#else
        return false;
#endif
    }
...
/**
 * The context belonging to a queue, owned for the run rather than by whoever asked first.
 *
 ...
 */
[[nodiscard]] GNURADIO_EXPORT inline DeviceContextSycl& syclContextFor(SyclQueue& queue) {
    // never destroyed, as the registry's own contexts: a DeviceContextSycl owns a queue by value, and destroying
    // one at exit races the CUDA driver's deinitialisation (~cuda_queue, CUDA error 4)
    static auto& mutex    = *new std::mutex();
    static auto& contexts = *new std::unordered_map<SyclQueue, std::unique_ptr<DeviceContextSycl>>();
