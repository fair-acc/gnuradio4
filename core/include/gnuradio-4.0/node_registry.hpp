...
class node_registry {
private:
    using node_type_handler = std::function<void(std::unique_ptr<gr::node_model> &, const property_map &)>;
    ...
    static auto
    create_handler() {
        return [](std::unique_ptr<gr::node_model> &result, const property_map &params) {
            using NodeType = NodeTemplate<NodeParameters...>;

            if constexpr (std::is_constructible_v<NodeType, const property_map &>) {
                result = std::make_unique<gr::node_wrapper<NodeType>>(params); // gp_plugin_node::wrap(std::make_shared<NodeType>(params));
            } else if constexpr (std::is_constructible_v<NodeType>) {
                result = std::make_unique<gr::node_wrapper<NodeType>>();
            } else {
                gr::meta::print_types<gr::meta::message_type<"Can not default-construct the node instance, nor to construct it from const property_map&">, NodeType>{};
            }
            return true;
        };
    }
...
public:
    ...
    create_node(std::string_view name, std::string_view type, const property_map &params) {
        std::unique_ptr<gr::node_model> result;
