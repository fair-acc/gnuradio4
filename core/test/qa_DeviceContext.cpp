...
const suite<"device::DeviceContextRegistry"> registryResolution = [] {
    ...
    "'host:native' is served in every build, and plain 'host' still is not"_test = [] {
        gr::device::DeviceContextRegistry& registry = gr::device::DeviceContextRegistry::instance();

        expect(registry.tryResolve("host:native") != nullptr) << "the CPU backing is served whatever backends were compiled in";
        expect(registry.tryResolve("host") == nullptr) << "plain host must stay unserved, or a downgraded device domain would silently dispatch to a CPU context";

        // and it is off the downgrade ladder: an unserved device domain walks declared -> un-indexed -> host:sycl,
        // and ends at plain 'host' when none of those is served. It must never land here.
        const gr::DomainResolution resolution = registry.resolve("fpga:nothing-serves-this");
        expect(resolution.resolved != "host:native"s) << std::format("the ladder must not reach the CPU backing, got '{}'", resolution.resolved);
    };
