...
struct BusyLoopBlock : public gr::Block<BusyLoopBlock<T>> {
    ...
    [[nodiscard]] constexpr gr::work::Status processBulk(gr::ConsumableSpan auto& input, gr::PublishableSpan auto& output) noexcept {
        ...
        if (produceCount == 0) {
            return gr::lifecycle::isActive(this->state()) ? gr::work::Status::INSUFFICIENT_OUTPUT_ITEMS : gr::work::Status::DONE;
        }
...
const boost::ut::suite<"SchedulerTests"> SchedulerTests = [] {
    ...
    */
    "blocking scheduler"_test = [&threadPool, &createWatchdog] {
        using namespace gr;
        using namespace gr::testing;
        using TScheduler = scheduler::Simple<scheduler::ExecutionPolicy::singleThreaded, profiling::null::Profiler, int, BlockingIO<>>;
        ...
        auto sched = TScheduler{std::move(flow), threadPool};

        expect(eq(0, sched.graph().progress().value())) << "initial progress definition (0)";
        auto schedulerThread = std::thread([&sched] {
            auto ret = sched.runAndWait();
            expect(ret.has_value()) << fmt::format("nested scheduler execution failed:\n{:f}\n", ret.error());
        });
        expect(awaitCondition(1s, [&sched] { return sched.state() == lifecycle::State::RUNNING; })) << "scheduler thread up and running w/ timeout";
        expect(sched.state() == lifecycle::State::RUNNING) << "scheduler thread up and running";
        expect(source.out.isConnected());
        expect(monitor.out.isConnected());

        fmt::println("##checked for source and monitor connections");

        expect(sched.state() == lifecycle::State::RUNNING) << "is running";
        std::this_thread::sleep_for(20ms);
        const auto progressAfterInit = sched.graph().progress().value();
        expect(eq(sched.graph().progress().value(), progressAfterInit)) << "after thread started definition (0)";
        monitor._produceCount.setValue(1L);
        std::this_thread::sleep_for(20ms);

        expect(eq(sched.graph().progress().value(), progressAfterInit + 3L)) << "final progress definition (>0)";
        fmt::println("final progress {}", sched.graph().progress().value());
