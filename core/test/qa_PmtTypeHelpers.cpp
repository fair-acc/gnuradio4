...
#include <gnuradio-4.0/PmtTypeHelpers.hpp> // Adjust include path as needed
...
const boost::ut::suite<"pmt safe conversion tests"> _conversionTests = [] {
    ...
    "integral->integral (in-range)"_test = [] {
        TestVariant v   = 42;
        auto        res = pmtv::safely_convert<std::int64_t>(v);
        expect(res.has_value()) << fmt::format("expected success, got: {}", res.error());
        if (res) {
    ...
    "integral->integral (out-of-range)"_test = [] {
        ...
        auto        res = pmtv::safely_convert<std::int16_t>(v);
        expect(!res.has_value()) << fmt::format("should fail, got: {}", res.error());
    };

    "signed->unsigned (negative)"_test = [] {
        TestVariant v   = -1;
        auto        res = pmtv::safely_convert<std::uint32_t>(v);
        expect(!res.has_value()) << fmt::format("negative -> unsigned should fail, got: {}", res.error());
    };

    "integral->float (within mantissa)"_test = [] {
        // 2^24 = 16777216 is the approximate integer limit for exact float
        TestVariant v   = (1 << 24) - 1;
        auto        res = pmtv::safely_convert<float>(v);
        expect(res.has_value()) << fmt::format("should succeed, got: {}", res.error());
        if (res) {
    ...
    "integral->float (bit_width check, out-of-range for float mantissa)"_test = [] {
        ...
        TestVariant v   = (1 << 24) + 1;
        auto        res = pmtv::safely_convert<float>(v);
        expect(!res.has_value()) << fmt::format("bit_width logic error, got: {}", res.error());
    };

    "float->integral (exact integer)"_test = [] {
        TestVariant v   = 42.0f;
        auto        res = pmtv::safely_convert<std::int32_t>(v);
        expect(res.has_value()) << fmt::format("should succeed for exact integer, got: {}", res.error());
        if (res) {
            expect(*res == 42);
        }
    };
    ...
    "float->integral (not exact integer)"_test = [] {
        TestVariant v   = 42.1f;
        auto        res = pmtv::safely_convert<std::int32_t>(v);
        expect(!res.has_value()) << fmt::format("should fail for non-integer, got: {}", res.error());
    };

    "float->integral (out-of-range)"_test = [] {
        TestVariant v   = 3e9f; // 3,000,000,000 outside 32-bit int range
        auto        res = pmtv::safely_convert<std::int32_t>(v);
        expect(!res.has_value()) << fmt::format("should fail out-of-range, got: {}", res.error());
    };

    "float->double (valid)"_test = [] {
        TestVariant v   = 1.2345f;
        auto        res = pmtv::safely_convert<double>(v);
        expect(res.has_value()) << fmt::format("Should succeed float->double, got: {}", res.error());
    };
    ...
    "string->integral (valid)"_test = [] {
        ...
        auto        res = pmtv::safely_convert<std::int32_t>(v);
        expect(res.has_value()) << fmt::format("should parse successfully, got: {}", res.error());
        if (res) {
    ...
    "string->integral (invalid)"_test = [] {
        ...
        auto        res = pmtv::safely_convert<std::int32_t>(v);
        expect(!res.has_value()) << fmt::format("should fail for float-like string, got: {}", res.error());
    };
    ...
    "string->float (valid)"_test = [] {
        ...
        auto        res = pmtv::safely_convert<float>(v);
        expect(res.has_value()) << fmt::format("should parse float, got: {}", res.error());
    };

    "string->float (invalid)"_test = [] {
        TestVariant v   = "not_a_float"s;
        auto        res = pmtv::safely_convert<double>(v);
        expect(!res.has_value()) << fmt::format("should fail parse, got: {}", res.error());
    };
    ...
    "string->float (out-of-range)"_test = [] {
        ...
        auto        res = pmtv::safely_convert<double>(v);
        expect(!res.has_value()) << fmt::format("should fail parse out-of-range, got: {}", res.error());
    };

    "string->bool (valid, true)"_test = [] {
        TestVariant v   = "TRUE"s;
        auto        res = pmtv::safely_convert<bool>(v);
        expect(res.has_value()) << fmt::format("should parse bool, got: {}", res.error());
        if (res) {
            expect(*res == true);
        }
    };
    ...
    "string->bool (invalid)"_test = [] {
        ...
        auto        res = pmtv::safely_convert<bool>(v);
        expect(!res.has_value()) << fmt::format("should fail parse bool, got: {}", res.error());
    };

    "bool->string (valid, true)"_test = [] {
        TestVariant v   = true;
        auto        res = pmtv::safely_convert<std::string>(v);
        expect(res.has_value()) << fmt::format("should succeed, got: {}", res.error());
        if (res) {
            expect(*res == "true"s);
        }
    ...
    "enum->string"_test = [] {
        ...
        auto        res = pmtv::safely_convert<std::string>(v);
        expect(res.has_value()) << fmt::format("should succeed enum->string, got: {}", res.error());
        if (res) {
            expect(*res == "Blue"s);
        }
    };
    ...
    "string->enum (valid)"_test = [] {
        ...
        auto        res = pmtv::safely_convert<test::Colour>(v);
        expect(res.has_value()) << fmt::format("should parse enum, got: {}", res.error());
        if (res) {
    ...
    "string->enum (invalid)"_test = [] {
        ...
        auto        res = pmtv::safely_convert<test::Colour>(v);
        expect(!res.has_value()) << fmt::format("Should fail invalid enum, got: {}", res.error());
    };

    "complex<float> -> complex<double>"_test = [] {
        TestVariant v   = std::complex<float>(1.25f, -2.5f);
        auto        res = pmtv::safely_convert<std::complex<double>>(v);
        expect(res.has_value()) << "should succeed: complex<float> -> complex<double>";
        if (res) {
    ...
    "complex<double> -> complex<float>"_test = [] {
        ...
        auto        res = pmtv::safely_convert<std::complex<float>>(v);
        expect(res.has_value()) << "should succeed: complex<double> -> complex<float>";
        if (res) {
    ...
    "integral -> complex<float>"_test = [] {
        ...
        auto        res = pmtv::safely_convert<std::complex<float>>(v);
        expect(res.has_value()) << "should succeed: int -> complex<float>";
        if (res) {
    ...
    "double -> complex<double>"_test = [] {
        TestVariant v   = 123.456;
        auto        res = pmtv::safely_convert<std::complex<double>>(v);
        expect(res.has_value()) << "should succeed: double -> complex<double>";
        if (res) {
    ...
    "complex<float> -> integral (not implemented)"_test = [] {
        ...
        auto        res = pmtv::safely_convert<int>(v);
        expect(!res.has_value()) << fmt::format("should fail, got: {}", res.error());
    };

    "complex<double> -> float (not implemented)"_test = [] {
