...
const boost::ut::suite<"ValueMap - key-type interop"> _key_interop_suite = [] {
    ...
    "insert / lookup / erase round-trip across all 6 string-like key types"_test = [] {
        ...
        map.emplace("sv"sv, std::uint32_t{3});             // std::string_view
        ...
        expect(eq(map.value_or<std::uint32_t>("sv"sv, 0U), std::uint32_t{3}));
        ...
        map.erase(map.find("char_lit"sv));
...
const boost::ut::suite<"ValueMap - STL container interop"> _stl_interop_suite = [] {
    ...
    "construct ValueMap from std::map<std::string, Value, std::less<>>"_test = [] {
        ...
        src.emplace("signal_name", Value{"carrier-A"sv});
        ...
        expect(eq(map.value_or<std::string_view>("signal_name", std::string_view{}), "carrier-A"sv));
    };
    ...
    "to_std_map round-trip preserves all entries"_test = [] {
        ...
        expect(eq(out.at("signal_name").value_or(std::string_view{}), "carrier-A"sv));
    };
    ...
    "size comparison: 20 canonical Tag entries — ValueMap blob vs std::pmr::map / unordered_map"_test = [] {
        ...
        vmap.emplace("signal_name", "carrier-A"sv);
        vmap.emplace("num_channels", std::uint32_t{4});
        vmap.emplace("signal_quantity", "voltage"sv);
        vmap.emplace("signal_unit", "V"sv);
        ...
        vmap.emplace("trigger_name", "PPS"sv);
        ...
        vmap.emplace("context", "default"sv);
        ...
        stdInsert(omap, "signal_name", Value{"carrier-A"sv});
        stdInsert(omap, "num_channels", Value{std::uint32_t{4}});
        stdInsert(omap, "signal_quantity", Value{"voltage"sv});
        stdInsert(omap, "signal_unit", Value{"V"sv});
        ...
        stdInsert(omap, "trigger_name", Value{"PPS"sv});
        ...
        stdInsert(omap, "context", Value{"default"sv});
        ...
        stdInsert(umap, "signal_name", Value{"carrier-A"sv});
        stdInsert(umap, "num_channels", Value{std::uint32_t{4}});
        stdInsert(umap, "signal_quantity", Value{"voltage"sv});
        stdInsert(umap, "signal_unit", Value{"V"sv});
        ...
        stdInsert(umap, "trigger_name", Value{"PPS"sv});
        ...
        stdInsert(umap, "context", Value{"default"sv});
...
#ifndef __EMSCRIPTEN__ // blob is byte-identical 32/64-bit; node-based maps shrink with 4-byte WASM pointers, flipping the margin at small N
...
const boost::ut::suite<"ValueMap - operator[] / typed at&lt;name&gt; / equal_range / merge"> _bracket_and_range_suite = [] {
    ...
    "SubscriptProxy: write accepts string/string_view"_test = [] {
        ValueMap map;
        map["signal_name"] = "carrier"sv;
        expect(eq(map["signal_name"].value_or(std::string_view{}), "carrier"sv));
    };
    ...
    "insert_or_assign(K, view-mode Value) byte-copies the source value-record"_test = [] {
        ...
        src.insert_or_assign("text", "hello, fast path"sv);
...
const boost::ut::suite<"ValueMap - edge cases"> _edge_case_suite = [] {
    ...
    "self-aliasing emplace via iterator key (string_view into _blob) does not crash"_test = [] {
        ...
        expect(eq(k, "source_inline_key"sv));
        map.emplace("new_key", k);
        expect(eq(map.size(), 2UZ));
        expect(eq(map.value_or<std::string_view>("new_key", std::string_view{}).size(), "source_inline_key"sv.size()));
    };
...
const boost::ut::suite<"ValueMap - extended value types (complex / nested ValueMap)"> _extended_types_suite = [] {
    ...
    "nested ValueMap round-trip — single-level (trigger_meta_info-style)"_test = [] {
        ValueMap inner;
        inner.emplace("inner_count", std::uint32_t{42});
        inner.emplace("inner_label", "abc"sv);
...
const boost::ut::suite<"ValueMap - Tensor support"> _tensor_suite = [] {
    ...
    "Tensor<Value> with String elements (settings-style string array)"_test = [] {
        ValueMap          map;
        gr::Tensor<Value> src(gr::extents_from, std::array<std::size_t, 1>{3UZ});
        src._data.data()[0] = Value{"alpha"sv};
        src._data.data()[1] = Value{"bravo"sv};
        src._data.data()[2] = Value{"charlie"sv};
        ...
        expect(eq(t->size(), 3UZ));
        expect(eq((*t)[0].value_or(std::string_view{}), "alpha"sv));
        expect(eq((*t)[1].value_or(std::string_view{}), "bravo"sv));
        expect(eq((*t)[2].value_or(std::string_view{}), "charlie"sv));
    };
    ...
    "Tensor<Value> elements may themselves be nested ValueMaps (recursive composition)"_test = [] {
        ...
        ValueMap          map;
        gr::Tensor<Value> src(gr::extents_from, std::array<std::size_t, 1>{2UZ});
        src._data.data()[0] = Value{"head"sv};
        ...
        expect(eq(head.value_or(std::string_view{}), "head"sv));
    ...
    "ValueMap containing a Tensor<float> coexists with sibling scalar entries"_test = [] {
        ...
        expect(eq(map.value_or<float>("sample_rate", 0.f), 48000.0f));
        expect(eq(map.value_or<std::string_view>("label", std::string_view{}), "capture"sv));
    };
...
const boost::ut::suite<"ValueMap - Tensor<Value> view contract"> _tensor_value_view_contract_suite = [] {
    ...
    "iterator yields exactly elementCount Values"_test = [] {
        ...
        src._data.data()[0] = Value{std::int64_t{-7}};
        src._data.data()[1] = Value{0.5f};
        src._data.data()[2] = Value{"three"sv};
    ...
    "random access [i] matches sequential decode"_test = [] {
        ...
        src._data.data()[1] = Value{"middle"sv};
        ...
        expect(eq((*view)[1].value_or(std::string_view{}), seq[1].value_or("X"sv)));
    ...
    "from_blob then get_if<TensorView<Value>> then owned(mr) preserves every cell"_test = [] {
        ...
        expect(eq(snap.data()[1].value_or(std::string_view{}), "middle"sv));
    ...
    "nested Tensor<Value>-of-Tensor<Value> round-trips through ValueMap blob"_test = [] {
        ...
        innerA._data.data()[1] = Value{"a1"sv};
        ...
        innerB._data.data()[2] = Value{"b3"sv};
        ...
        expect(eq((*innerViewA)[1].value_or(std::string_view{}), "a1"sv));
    ...
    "owned(mr) snapshot is independent of the source ValueMap's lifetime"_test = [] {
        ...
        expect(eq(snap.data()[0].value_or(std::string_view{}), "alpha"sv));
    ...
    "rank-0 (scalar tensor) with String element decodes cleanly"_test = [] {
        ...
        src._data.data()[0] = Value{"answer"sv};
        ...
        expect(eq((*view)[0].value_or(std::string_view{}), "answer"sv));
    };
    ...
    "Tensor<Value> of ValueMap-with-strings — iter+get_if path mirroring loadGraphFromMap"_test = [] {
        ...
        expect(eq(innerA->template value_or<std::string_view>("name", ""), "multiplier1"sv));
        expect(eq(innerA->template value_or<std::string_view>("type", ""), "good::multiply<float64>"sv));

        const auto innerB = outerSnap.data()[1].get_if<ValueMap>();
        expect(innerB.has_value()) << boost::ut::fatal;
        expect(eq(innerB->template value_or<std::string_view>("name", ""), "sink"sv));
        expect(eq(innerB->template value_or<std::string_view>("type", ""), "good::cout_sink<float64>"sv));
    };
};
...
const boost::ut::suite<"ValueMap - erase / clear / reserve / shrink"> _mutation_suite = [] {
    ...
    "clear removes all entries and resets payload usage"_test = [] {
        ValueMap map;
        map.insert_or_assign("sample_rate", 48000.0f);
        map.emplace("s", "some-string-longer-than-SSO-for-robustness"sv);
        expect(eq(map.size(), 2UZ));
        map.clear();
        expect(eq(map.size(), 0UZ));
        expect(map.empty());
    ...
    "growth across the initial capacity preserves already-inserted entries"_test = [] {
        ...
        expect(eq(map.size(), 4UZ));
        for (const auto& [key, val] : {std::pair{"a"sv, std::uint32_t{10}}, //
                 std::pair{"b"sv, std::uint32_t{20}}, std::pair{"c"sv, std::uint32_t{30}}, std::pair{"d"sv, std::uint32_t{40}}}) {
    ...
    "string values survive blob relocation across growth"_test = [] {
        ...
        map.emplace("first", "sample-carrier-name-01"sv);
        map.emplace("second", "sample-carrier-name-02"sv);
        map.emplace("third", "sample-carrier-name-03"sv);
        expect(eq(map.size(), 3UZ));
        expect(eq(map.value_or<std::string_view>("first", ""sv), "sample-carrier-name-01"sv));
        expect(eq(map.value_or<std::string_view>("second", ""sv), "sample-carrier-name-02"sv));
        expect(eq(map.value_or<std::string_view>("third", ""sv), "sample-carrier-name-03"sv));
    };
    ...
    "insert_or_assign on same key reclaims old payload (C2 regression)"_test = [] {
        ValueMap map;
        map.insert_or_assign("key", "first content here"sv);
        map.shrink_to_fit();
        const auto baselineBlobSize = map.blob().size();
        for (int i = 0; i < 200; ++i) {
            map.insert_or_assign("key", "replacement value with content"sv);
        }
        map.shrink_to_fit();
    ...
    "erase string + split-fit reuse leaves neighbour record intact (C1 regression)"_test = [] {
        ...
        map.insert_or_assign("a", "abcdefgh"sv);
...
const boost::ut::suite<"ValueMap - iteration"> _iter_suite = [] {
    ...
    "reverse iteration visits entries last-to-first"_test = [] {
        ...
        expect(eq(seen[0], "c"sv));
        expect(eq(seen[1], "b"sv));
        expect(eq(seen[2], "a"sv));
    };
    ...
    "random-access arithmetic and operator[] on const_iterator"_test = [] {
        ...
        expect(eq((first + 1)[1].first, "c"sv)); // first[2]
...
const boost::ut::suite<"ValueMap - copy / move semantics"> _copy_move_suite = [] {
    ...
    "copy construction deep-copies into the target resource"_test = [] {
        ...
        src.emplace("note", "hello-world-this-string-is-long"sv);
        ...
        expect(eq(copy.value_or<std::string_view>("note", ""sv), "hello-world-this-string-is-long"sv));
    };
...
const boost::ut::suite<"ValueMap - wire format"> _wire_format_suite = [] {
    ...
    "from_blob round-trip preserves scalar / string / unsigned-integer / tensor entries"_test = [] {
        ...
        src.emplace("signal_name", "demo"sv);
        ...
        if (restored) {
            ...
            expect(eq(restored->value_or<std::string_view>("signal_name", std::string_view{}), "demo"sv));
    ...
    "'!EOD\\0' marker lands at payloadOffset + payloadUsed (debug only)"_test = [] {
        ...
        ValueMap map;
        map.insert_or_assign("k", "some-payload-string"sv);
        const auto       blob = map.blob();
        const auto*      hdr  = std::launder(reinterpret_cast<const gr::pmt::Header*>(blob.data()));
...
const boost::ut::suite<"ValueMap - view-mode (makeView / is_view / owned)"> _view_mode_suite = [] {
    ...
    "makeView aliases external bytes; is_view() reports true"_test = [] {
        ...
        src.emplace("signal_name", "hello"sv);
        const auto bytes = src.blob();
    ...
    "owned(resource) materialises a fresh allocation independent of the source bytes"_test = [] {
        ...
        expect(eq(owned.value_or<std::string_view>("note", std::string_view{}), "abc"sv));
    };
...
const boost::ut::suite<"ValueMap iter view-mode (variable-size value types)"> _iter_view_mode_suite = [] {
    ...
    "iter yields correct nested ValueMap (V2: owning, view-mode pending)"_test = [] {
        ...
        inner.insert_or_assign("b", "hello"sv);
        ValueMap outer;
    ...
    "iter yields correct Tensor<Value> (variable-size cells)"_test = [] {
        ...
        for (const auto& [k, v] : map) {
            ...
            if (view) {
                ...
                expect(eq((*view)[1].value_or(std::string_view{}), "x"sv));
            }
        }
        expect(eq(seen, 1UZ));
    };
...
const boost::ut::suite<"ValueMap - monadic lookup (find_value / get_if / value_or / or_else / holds)"> _monadic_suite = [] {
    ...
    "get_if<string_view>: hit returns view aliasing blob"_test = [] {
        ValueMap m;
        m.insert_or_assign("name", "hello"sv);
        const auto sv = m.get_if<std::string_view>("name");
        expect(sv.has_value());
        if (sv) {
            expect(eq(*sv, "hello"sv));
        }
    };
    ...
    "value_or<string_view>: alloc-free view-mode read"_test = [] {
        ValueMap m;
        m.insert_or_assign("name", "world"sv);
        const auto sv = m.value_or<std::string_view>("name", "");
        expect(eq(sv, "world"sv));
    };
    ...
    "ArrowProxy: it->second works (chained-arrow rule)"_test = [] {
        ...
        expect(it != m.end());
        if (it != m.end()) {
            expect(eq(std::string_view{it->first}, "rate"sv));
...
const boost::ut::suite<"ValueMap - shared-buffer round-trip (USM/IPC-style)"> _shared_buffer_suite = [] {
    ...
    "host writes via ValueMap, kernel reads via ValueMapView (slice through public base)"_test = [] {
        ...
        std::ignore = host.try_emplace("label", "alpha"sv);
        ...
        for (auto [key, val] : kernelView) {
            ...
            if (key == "counter") {
                ...
                expect(eq(*val.get_if<std::string_view>(), "alpha"sv));
            }
        }
        expect(eq(seen, 3UZ));
...
const boost::ut::suite<"ValueMapView - bounded mutators (try_emplace / insert_or_assign / erase / clear)"> _vm_view_mutator_suite = [] {
    ...
    "try_emplace<std::string_view> appends a String entry with NUL-terminated payload"_test = [] {
        ...
        host.reserve(8U, 256U);
        ValueMapView& view = host;
        expect(view.try_emplace("unit", "Hz"sv));
        const auto v = host.find_value("unit");
        expect(v.has_value());
        expect(v->holds<std::string_view>());
        expect(eq(*v->get_if<std::string_view>(), "Hz"sv));
    };
    ...
    "try_emplace<std::string_view> fails when payload pool cannot fit the record"_test = [] {
        ...
        expect(!view.try_emplace("long", "abcdef"sv));
    };

    "insert_or_assign<std::string_view> overwrites equal-length string in place"_test = [] {
        ValueMap host;
        host.reserve(8U, 256U);
        std::ignore        = host.try_emplace("name", "ABC"sv);
        ValueMapView& view = host;
        expect(view.insert_or_assign("name", "XYZ"sv));
        expect(eq(*host.find_value("name")->get_if<std::string_view>(), "XYZ"sv));
    };

    "insert_or_assign<std::string_view> succeeds for two short strings sharing the same padded record"_test = [] {
        ValueMap host;
        host.reserve(8U, 256U);
        std::ignore        = host.try_emplace("k", "ab"sv); // recSize = max(16, 8+2+1) = 16
        ValueMapView& view = host;
        expect(view.insert_or_assign("k", "de"sv)); // same recSize → in-place
        expect(view.insert_or_assign("k", ""sv));   // empty still fits → recSize 16
        expect(eq(*host.find_value("k")->get_if<std::string_view>(), ""sv));
    };

    "insert_or_assign<std::string_view> rejects size-changing overwrites"_test = [] {
        ValueMap host;
        host.reserve(8U, 256U);
        std::ignore        = host.try_emplace("k", "longer-than-seven-chars"sv); // recSize > 16
        ValueMapView& view = host;
        // shorter → smaller recSize → bounded path must reject (would require record shrinkage)
        expect(!view.insert_or_assign("k", "short"sv));
        expect(eq(*host.find_value("k")->get_if<std::string_view>(), "longer-than-seven-chars"sv));
    };
    ...
    "insert_or_assign<std::string_view> rejects type-mismatch (existing entry is non-String)"_test = [] {
        ...
        ValueMapView& view = host;
        expect(!view.insert_or_assign("k", "hi"sv));
    ...
    "insert_or_assign<std::string_view> inserts new key into empty map"_test = [] {
        ...
        host.reserve(8U, 256U);
        ValueMapView& view = host;
        expect(view.insert_or_assign("greeting", "hi"sv));
        expect(eq(*host.find_value("greeting")->get_if<std::string_view>(), "hi"sv));
    };
...
const boost::ut::suite<"ValueMap::makeAt + growing try_emplace (ring-writer prerequisites)"> _vm_make_at_suite = [] {
    ...
    "insert_or_assign(K, Value) on a fixed buffer reports failure (not phantom success) when the record does not fit"_test = [] {
        // regression: _insertNew evaluated end() before the scope-exit rollback ran, so a rejected
        // fixed-buffer insert wrongly returned {it, true} while the entry count was rolled back to 0.
        alignas(16) std::array<std::byte, 256> slot{};
        auto                                   map = ValueMap::makeAt(slot, /*payloadCapacity=*/16U, /*entryCapacity=*/2U);

        gr::pmt::Value big{std::string(512, 'x')}; // record far larger than the 16-B payload pool
        const auto [it, ok] = map.insert_or_assign("payload"sv, big);
        expect(!ok) << "a rejected fixed-buffer insert must report failure";
        expect(it == map.end()) << "the failure iterator must equal end()";
        expect(eq(map.size(), 0UZ)) << "a rejected insert must not leave a phantom entry";
    };
...
const boost::ut::suite<"Value::get_if<ValueMap> lifetime contract"> _value_get_if_value_map_lifetime_suite = [] {
    ...
    auto makeNestedMap = [](std::pmr::memory_resource* mr = std::pmr::get_default_resource()) {
        ...
        std::ignore = m.insert_or_assign("b", "hello"sv);
        return m;
    };
    ...
    "owned() materialisation survives source destruction"_test = [&] {
        ...
        expect(b.has_value());
        expect(eq(b->template get_if<std::string_view>().value_or(""), "hello"sv));
    };
...
const boost::ut::suite<"ValueMap - a nested map built in place"> _nested_in_place_suite = [] {
    ...
    "a nested map refuses rather than overflows when the 16-aligned record size cannot fit in uint32_t"_test = [] {
        alignas(gr::pmt::kBlobAlignment) std::byte storage[2048];
        gr::pmt::ValueMapView                      parent = gr::pmt::ValueMapView::formatAt(std::span<std::byte>(storage, sizeof(storage)), 640U, 8U);
        expect(parent._header != nullptr);
        if (parent._header == nullptr) {
        ...
        // childBytes = sizeof(Header) + 1 * sizeof(PackedEntry) + payloadBytes = 80 + 4294967199 = 4294967279, so the
        // coarse recBytes = 16 + childBytes check lands exactly on UINT32_MAX (and passes), while the true
        // kRecAlignment-rounded record size rolls over to 2^32 and must still be refused, not wrapped.
        const gr::pmt::ValueMapView child = parent.try_emplace_map(std::string_view{"huge"}, 0U, 4294967199U);
        expect(child._header == nullptr) << "a record size overflowing uint32_t after alignment must be refused, not silently truncated";
        expect(eq(parent.size(), 0UZ)) << "a refused nesting must not consume an entry slot";
    };
...
const boost::ut::suite<"ValueMap - alloc-free typed reads"> _alloc_free_read_suite = [] {
    ...
    "a string read is delimited by its terminator, not by the slot it occupies"_test = [] {
        gr::pmt::ValueMap map;
        map.insert_or_assign("name", std::string_view{"a-fairly-long-signal-name"});
        map.insert_or_assign("name", std::string_view{"short"});

        // the overwrite reuses the wider slot, so payloadLength still measures the slot
        const auto read = map.get_if<std::string_view>("name");
        expect(read.has_value()) << boost::ut::fatal;
    ...
    "a typed read does not mistake a tensor for one of its elements"_test = [] {
        ...
        gr::Tensor<float> samples(gr::extents_from, std::array<std::size_t, 1>{4UZ});
        std::ranges::copy(std::array<float, 4>{1.f, 2.f, 3.f, 4.f}, samples._data.begin());
        expect(map.try_emplace(std::string_view{"samples"}, samples));

        // a tensor records its ELEMENT type in the entry, so a scalar read that trusts that field alone
        // hands back a pointer into the tensor's own header
        expect(map.get_if<float>(std::string_view{"samples"}) == nullptr) << "a Tensor<float> is not a float";
        expect(map.get_if<gr::TensorView<float>>(std::string_view{"samples"}).has_value()) << "and it is still readable as what it is";
    };
};
