...
const boost::ut::suite DynamicBlocktests = [] {
    ...
    "Change number of ports dynamically"_test = [] {
        ...
        for (std::size_t i = 0; i < nInputs; ++i) {
            ...
            expect(gr::ConnectionResult::SUCCESS == graph.connect2(*sources.back(), "out"s, adder, "in#"s + std::to_string(sources.size() - 1)));
        }
        expect(gr::ConnectionResult::SUCCESS == graph.connect2(adder, adder.out, sink, sink.in));
