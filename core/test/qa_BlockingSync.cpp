...
const boost::ut::suite<"Graph Integration"> graphIntegration = [] {
    "clock source to sink"_test = [] {
        ...
        expect(eq(ConnectionResult::SUCCESS, testGraph.connect<"out", "in">(src, sink)));
    ...
    "generator free-running mode"_test = [] {
        ...
        expect(eq(ConnectionResult::SUCCESS, testGraph.connect<"out", "in">(gen, sink)));
    ...
    "generator clock-driven mode"_test = [] {
        ...
        expect(eq(ConnectionResult::SUCCESS, testGraph.connect<"out", "clk_in">(clock, gen)));
        expect(eq(ConnectionResult::SUCCESS, testGraph.connect<"out", "in">(gen, sink)));
