...
const boost::ut::suite<"Graph Integration"> graphIntegration = [] {
    "clock source to sink"_test = [] {
        ...
        expect(eq(ConnectionResult::SUCCESS, testGraph.connect(src, src.out, sink, sink.in)));
    ...
    "generator free-running mode"_test = [] {
        ...
        expect(eq(ConnectionResult::SUCCESS, testGraph.connect(gen, gen.out, sink, sink.in)));
    ...
    "generator clock-driven mode"_test = [] {
        ...
        expect(eq(ConnectionResult::SUCCESS, testGraph.connect(clock, clock.out, gen, gen.clk_in)));
        expect(eq(ConnectionResult::SUCCESS, testGraph.connect(gen, gen.out, sink, sink.in)));
