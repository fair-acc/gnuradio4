...
const boost::ut::suite SettingsTests = [] {
    ...
    "basic node settings tag"_test = [] {
        ...
        expect(eq(ConnectionResult::SUCCESS, testGraph.connect2(src, src.out, block1, block1.in)));
        expect(eq(ConnectionResult::SUCCESS, testGraph.connect2(block1, block1.out, block2, block2.in)));
        expect(eq(ConnectionResult::SUCCESS, testGraph.connect2(block2, block2.out, sink, sink.in)));
...
const boost::ut::suite CtxSettingsTests = [] {
    ...
    "CtxSettings autoUpdateParameters"_test = [&] {
        ...
        expect(eq(ConnectionResult::SUCCESS, testGraph.connect2(monitorOne, monitorOne.out, sinkBulk, sinkBulk.in)));
