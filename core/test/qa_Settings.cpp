...
const boost::ut::suite SettingsTests = [] {
    ...
    "basic node settings tag"_test = [] {
        ...
        expect(eq(src.settings().defaultParameters().size(), 11UZ)); // 7 base + 2 derived
        ...
        expect(eq(sink.settings().defaultParameters().size(), 11UZ)); // 8 base + 2 derived
        ...
        expect(eq(sink.settings().autoUpdateParameters().size(), 7UL)); // 3 base + 2 derived
