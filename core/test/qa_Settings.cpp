...
const boost::ut::suite SettingsTests = [] {
    ...
    "vector-type support"_test = [] {
        ...
        // expect(eq(block.string_vector_setting.value, Tensor<pmt::Value>{"A", "B", "C"}));
    ...
    "basic store/reset settings"_test = []() {
        ...
        expect(eq(gr::testing::get_value_or_fail<float>(defaultParOld.at("scaling_factor")), 2.f));
...
const boost::ut::suite AnnotationTests = [] {
    ...
    "basic node annotations"_test = [] {
        ...
        expect(gr::testing::get_value_or_fail<bool>(block.meta_information.value.at("scaling_factor::visible"))) << "visible being true";
...
const boost::ut::suite CtxSettingsTests = [] {
    ...
    "CtxSettings Time"_test = [] {
        ...
        // one key
        expect(eq(gr::testing::get_value_or_fail<float>(settings.getStored("scaling_factor").value()), 10.f));          // return as ctx.time = std::chrono::system_clock::now()
        expect(settings.getStored("scaling_factor", ctxM1) == std::nullopt);                                            // return std::nullopt, all settings ctx times are in the future
        expect(eq(gr::testing::get_value_or_fail<float>(settings.getStored("scaling_factor", ctx0).value()), 10.f));    // return exact
        expect(eq(gr::testing::get_value_or_fail<float>(settings.getStored("scaling_factor", ctx1).value()), 10.f));    // return previous
        expect(eq(gr::testing::get_value_or_fail<float>(settings.getStored("scaling_factor", ctx2).value()), 12.f));    // return exact
        expect(eq(gr::testing::get_value_or_fail<float>(settings.getStored("scaling_factor", ctx3).value()), 12.f));    // return previous
        expect(eq(gr::testing::get_value_or_fail<float>(settings.getStored("scaling_factor", ctx4).value()), 14.f));    // return exact
        expect(eq(gr::testing::get_value_or_fail<float>(settings.getStored("scaling_factor", ctx5).value()), 14.f));    // return latest
        expect(eq(gr::testing::get_value_or_fail<float>(settings.getStored("scaling_factor", ctxNull).value()), 10.f)); // return as ctx.time = std::chrono::system_clock::now()
        // several keys
        ...
        expect(eq(gr::testing::get_value_or_fail<float>(params.at("scaling_factor")), 10.f));
        ...
        expect(eq(gr::testing::get_value_or_fail<float>(params0.at("scaling_factor")), 10.f));
        ...
        expect(eq(gr::testing::get_value_or_fail<float>(params1.at("scaling_factor")), 10.f));
        ...
        expect(eq(gr::testing::get_value_or_fail<float>(params2.at("scaling_factor")), 12.f));
        ...
        expect(eq(gr::testing::get_value_or_fail<float>(params3.at("scaling_factor")), 12.f));
        ...
        expect(eq(gr::testing::get_value_or_fail<float>(params4.at("scaling_factor")), 14.f));
        ...
        expect(eq(gr::testing::get_value_or_fail<float>(params5.at("scaling_factor")), 14.f));
        ...
        expect(eq(gr::testing::get_value_or_fail<float>(paramsNull.at("scaling_factor")), 10.f));
    ...
    "CtxSettings Resolve Duplicate Timestamp"_test = [] {
        ...
        expect(eq(gr::testing::get_value_or_fail<float>(settings.getStored("scaling_factor").value()), 12.f));
    };
#endif
    ...
    "CtxSettings Matching"_test = [&] {
        ...
        expect(eq(gr::testing::get_value_or_fail<int>(settings.getStored("scaling_factor", ctx1).value()), 102));
    ...
    "CtxSettings autoUpdateParameters"_test = [&] {
        ...
        auto testStored = [&](BlockLike auto& block) {
            ...
            expect(eq(gr::testing::get_value_or_fail<float>(vec[0].settings.at(gr::tag::SAMPLE_RATE.shortKey())), 1000.f)); // Parameters changed via Tag are not changed in the storedParameters
  ...
  - [test_block, 0, sink, 0]
        ...
        try {
            ...
            gr::graph::forEachBlock<gr::block::Category::NormalBlock>(sched.graph(), [](std::shared_ptr<BlockModel> block) { //
                expect(eq(gr::testing::get_value_or_fail<float>(*block->settings().get(gr::tag::SAMPLE_RATE.shortKey())), 123456.f)) << std::format("sample_rate forwarded to {}", block->name());
            });
