...
struct DirectFirSycl : Block<DirectFirSycl<T>, Resampling<>, Stride<>> {
    ...
    [[nodiscard]] gr::work::Status processBulk(gr::device::DeviceContext& ctx, InputSpanLike auto& input, OutputSpanLike auto& output) const {
...
struct DynamicCombinerWithHatch : Block<DynamicCombinerWithHatch<T>> {
    ...
    template<gr::InputSpanLike TInSpan>
    [[nodiscard]] gr::work::Status processBulk(gr::device::DeviceContext&, const std::vector<TInSpan>& ins, OutputSpanLike auto& output) const {
        for (std::size_t i = 0UZ; i < output.size(); ++i) {
            T sum{};
            for (const TInSpan& channel : ins) {
                sum += channel[i];
...
static_assert(!gr::traits::block::stream_input_ports<DynamicCombiner<float>>::template none_of<gr::traits::port::is_dynamic_port_collection>, "DynamicCombiner in-ports must read as a dynamic collection, or the device gate below tests nothing");
