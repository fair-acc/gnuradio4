...
const boost::ut::suite TagPropagation = [] {
    ...
    "TagPropagation autoForwardParameters constructor"_test = [&] {
        ...
        expect(eq(ConnectionResult::SUCCESS, testGraph.connect2(src, src.out, autoForwardBlock, autoForwardBlock.in)));
        expect(eq(ConnectionResult::SUCCESS, testGraph.connect2(autoForwardBlock, autoForwardBlock.out, monitor, monitor.in)));
        expect(eq(ConnectionResult::SUCCESS, testGraph.connect2(monitor, monitor.out, sink, sink.in)));
    ...
    auto runTest = []<auto srcType>(bool verbose = true) {
        ...
        //                                                                       └─> sinkOne
        expect(eq(ConnectionResult::SUCCESS, testGraph.connect2(src, src.out, monitorOne, monitorOne.in)));
        expect(eq(ConnectionResult::SUCCESS, testGraph.connect2(monitorOne, monitorOne.out, monitorBulk1, monitorBulk1.in)));
        expect(eq(ConnectionResult::SUCCESS, testGraph.connect2(monitorBulk1, monitorBulk1.out, autoForwardBlock, autoForwardBlock.in)));
        expect(eq(ConnectionResult::SUCCESS, testGraph.connect2(autoForwardBlock, autoForwardBlock.out, monitorBulk2, monitorBulk2.in)));
        expect(eq(ConnectionResult::SUCCESS, testGraph.connect2(monitorBulk2, monitorBulk2.out, sinkOne, sinkOne.in)));
        expect(eq(ConnectionResult::SUCCESS, testGraph.connect2(monitorBulk2, monitorBulk2.out, sinkBulk, sinkBulk.in)));
    ...
    "CustomTagHandling"_test = []() {
        ...
        expect(eq(ConnectionResult::SUCCESS, testGraph.connect2(src, src.out, realign, realign.inPort)));
        expect(eq(ConnectionResult::SUCCESS, testGraph.connect2(realign, realign.outPort, sink, sink.in)));
    ...
    auto runPolicyTest = []<typename TDecimator>(const std::vector<Tag>& expectedTags) {
        ...
        expect(eq(ConnectionResult::SUCCESS, testGraph.connect2(src, src.out, decimator, decimator.in)));
        expect(eq(ConnectionResult::SUCCESS, testGraph.connect2(decimator, decimator.out, sink, sink.in)));
...
const boost::ut::suite RepeatedTags = [] {
    ...
    auto runTest = []<auto srcType>(bool verbose = true) {
        ...
        expect(eq(ConnectionResult::SUCCESS, testGraph.connect2(src, src.out, monitorOne, monitorOne.in)));
        expect(eq(ConnectionResult::SUCCESS, testGraph.connect2(monitorOne, monitorOne.out, sinkOne, sinkOne.in)));
