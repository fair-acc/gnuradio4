...
const boost::ut::suite<"Value - Basic Construction"> _basic_construction_suite = [] {
    ...
    "a value compares against either complex spelling"_test = [] {
        Value v{gr::complex<float>{1.f, 2.f}};
        expect(v == gr::complex<float>{1.f, 2.f});
        expect(gr::complex<float>{1.f, 2.f} == v);
    };
...
const boost::ut::suite<"PmtTypeHelpers"> pmtTypeHelpersTests = [] {
    ...
    "convert_safely between complex Tensor spellings preserves multi-dimensional shape"_test = [] {
        gr::Tensor<std::complex<float>> src(gr::extents_from, std::array<std::size_t, 2>{2UZ, 3UZ});
        for (std::size_t i = 0UZ; i < src.size(); ++i) {
            src.data()[i] = std::complex<float>{static_cast<float>(i), static_cast<float>(-static_cast<int>(i))};
        }

        const auto converted = gr::pmt::convert_safely<gr::Tensor<gr::complex<float>>>(src);
        expect(converted.has_value()) << [&converted] { return converted.error(); } << fatal;
        expect(eq(converted->rank(), 2UZ));
        expect(eq(converted->extent(0), 2UZ));
        expect(eq(converted->extent(1), 3UZ));
        for (std::size_t i = 0UZ; i < src.size(); ++i) {
            expect(eq(converted->data()[i].real(), src.data()[i].real()));
            expect(eq(converted->data()[i].imag(), src.data()[i].imag()));
        }
    };
};
