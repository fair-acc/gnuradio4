...
/**
 * CPU stand-in for a device syncGroup: a ScheduledBlockGroup that runs its members from its own work().
 *
...
struct PassthroughDispatchScheduler : gr::Block<PassthroughDispatchScheduler> {
    ...
    [[nodiscard]] work::Result work(std::size_t requestedWork, gr::device::DeviceContext& computeBackend = gr::device::hostBackend()) noexcept {
        if (_quiescent) {
        ...
        for (auto& member : _graph->blocks()) {
            performed += member->work(requestedWork, computeBackend).performed_work;
            ++_memberCalls;
        }
