...
const boost::ut::suite GraphMessageTests = [] {
    ...
    "BlockRegistry tests"_test = [] {
        ...
        "Get available block types"_test = [&] {
            ...
            const Message reply = consumeFirstReply(fromGraph);

            if (reply.data.has_value()) {
                const auto& dataMap    = reply.data.value();
                auto        foundTypes = dataMap.find("types");
                if (foundTypes != dataMap.end()) {
                    const Value typesEntry = (*foundTypes).second; // bind to lvalue; ValueMap iter yields by value
                    if (typesEntry.holds<Tensor<Value>>()) {
