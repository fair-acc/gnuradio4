...
const boost::ut::suite<"New connection API tests"> connection_api_tests = [] {
    ...
    "Graph connection buffer size test - default"_test = [] {
        ...
        expect(eq(ConnectionResult::SUCCESS, graph.connect<"out", "in">(src, sink)));
...
const boost::ut::suite<"GraphTests"> _1 = [] {
    ...
    "Graph connection buffer size test - default"_test = [] {
        ...
        expect(eq(ConnectionResult::SUCCESS, graph.connect<"out", "in">(src, sink, {.minBufferSize = undefined_size})));
        graph.connectPendingEdges();
    ...
    "Graph connection buffer size test - set, one"_test = [] {
        ...
        expect(eq(ConnectionResult::SUCCESS, graph.connect<"out", "in">(src, sink, {.minBufferSize = 8000UZ})));
        graph.connectPendingEdges();
    ...
    "Graph connection buffer size test - set, many"_test = [] {
        ...
        expect(eq(ConnectionResult::SUCCESS, graph.connect<"out", "in">(src, sink1, {.minBufferSize = 2000UZ})));
        expect(eq(ConnectionResult::SUCCESS, graph.connect<"out", "in">(src, sink2, {.minBufferSize = 10000UZ})));
        expect(eq(ConnectionResult::SUCCESS, graph.connect<"out", "in">(src, sink3, {.minBufferSize = 8000UZ})));
...
const boost::ut::suite<"GraphExtensionsTests"> _2 = [] {
    ...
    "containsEdge returns true after connection"_test = [] {
        ...
        NullSink<float>&   snk = graph.emplaceBlock<NullSink<float>>();
        expect(eq(graph.connect<"out", "in">(src, snk), ConnectionResult::SUCCESS));
    ...
    "forEachEdge visits all edges"_test = [] {
        ...
        expect(eq(ConnectionResult::SUCCESS, graph.connect<"out", "in">(src, snk, {.minBufferSize = undefined_size})));
        graph.connectPendingEdges();
