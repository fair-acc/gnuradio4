...
struct SyclGain : Block<SyclGain> {
    ...
    [[nodiscard]] gr::work::Status processBulk(gr::device::DeviceContext& ctx, InputSpanLike auto& input, OutputSpanLike auto& output) {
        const std::size_t count = std::min(input.size(), output.size());
        ctx.copy(output.data(), input.data(), count * sizeof(float));
...
[[nodiscard]] bool domainIsServed(std::string_view domain) { return gr::device::DeviceContextRegistry::instance().isServedExactly(domain); }
...
int main() {
    ...
    "a block whose type no tier can take refuses the served domain instead of running somewhere else"_test = [syclAvailable] {
        ...
        gr::Graph flow;
        auto&     source = flow.emplaceBlock<TagSource<float, ProcessFunction::USE_PROCESS_BULK>>({{"n_samples_max", kN}, {"mark_tag", false}});
        // '!' is what turns "this block's type cannot reach the device" from a warning into a stop; without it the
        // settled policy is to fall back to the host, which is what the neighbouring case asserts
    ...
    "style 3: processBulk(ctx, ...) owns the queue and runs on the host thread"_test = [] {
        constexpr gr::Size_t kN     = 32U;
