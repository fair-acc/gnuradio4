...
std::vector<float> runGroup(gr::SubGraphHandle group, gr::Size_t nSamples = kSamples);
...
std::vector<float> runGroup(gr::SubGraphHandle group, gr::Size_t nSamples) {
    gr::Graph outer;
...
int main() {
    ...
    "makeDeviceSubGraph builds by itself what the manual wiring above builds by hand"_test = [] {
        ...
        const auto group = gr::basic::makeDeviceSubGraph<float>(std::move(inner));
        expect(group.has_value()) << [&] { return group ? std::string{} : group.error().message; };
        if (!group) {
            return;
        }
    ...
    "makeDeviceSubGraph gives a member with no domain of its own the group's"_test = [] {
        std::ignore                                  = gr::device::registerSyclRuntime();
        const std::optional<std::string_view> served = gr::test::firstServedSyclDomain();
        if (!served) {
        ...
        auto group = gr::basic::makeDeviceSubGraph<float>(std::move(inner));
        expect(group.has_value()) << [&] { return group ? std::string{} : group.error().message; };
        if (!group) {
            return;
        }
        ...
        for (const std::shared_ptr<gr::BlockModel>& member : static_cast<gr::SubGraphWrapper*>(group->block.get())->blockRef().graph().blocks()) {
    ...
    "two device groups chain device-to-device, with no host hop between them"_test = [] {
        ...
        const auto deviceGroup = [&]() {
            ...
            std::ignore          = only;
            return gr::makeSubGraph(std::move(inner));
        };
