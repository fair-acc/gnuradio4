...
const boost::ut::suite<"device execution"> device_execution = [] {
    using namespace boost::ut;

    "device-eligible block on gpu:sycl runs through registered runtime or CPU fallback"_test = [] {
        using namespace gr::testing;

        constexpr gr::Size_t kN             = 16U;
        const bool           syclRegistered = gr::device::registerSyclRuntime();
        auto&                deviceSched    = gr::device::SchedulerRegistry::instance().resolve("gpu:sycl");
#if GR_DEVICE_HAS_SYCL_IMPL
        expect(syclRegistered);
        expect(deviceSched.backend() == gr::device::DeviceBackend::SYCL);
#else
        expect(!syclRegistered);
        expect(deviceSched.backend() == gr::device::DeviceBackend::CPU_Fallback);
#endif

        gr::Graph        flow;
        auto&            source = flow.emplaceBlock<TagSource<float, ProcessFunction::USE_PROCESS_BULK>>({{"n_samples_max", kN}, {"mark_tag", false}});
        auto&            dut    = flow.emplaceBlock<gr::test::Copy>({{"gr:compute_domain", "gpu:sycl"}});
        ...
        expect(sched.runAndWait().has_value()) << "device-eligible block with compute_domain=gpu:sycl must complete";
