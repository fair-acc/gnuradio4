 ...
 *
 * Publishing a tag is the one thing a kernel cannot do (a tag payload is not device-constructible): the span
 * records the attempt and the dispatcher refuses the block once the kernel returns. The block is therefore written
 * the natural way and must produce identical tags on every domain -- which is what this asserts.
 */
...
/// Every SYCL domain reports "device": SSCP compiles one generic kernel whatever executes it, so this
