...
struct TemplatedSyclBulk : Block<TemplatedSyclBulk> {
    ...
    [[nodiscard]] work::Status processBulk(gr::device::DeviceContext&, InputSpanLike auto& input, OutputSpanLike auto& output) {
        std::ignore = input.consume(0);
        output.publish(0);
        return work::Status::OK;
    }
...
struct TwoInputSyclBulk {
    [[nodiscard]] work::Status processBulk(gr::device::DeviceContext&, InputSpanLike auto& inputA, InputSpanLike auto& inputB, OutputSpanLike auto& output) {
