...
const boost::ut::suite<"gr::thread_pool Manager WASM"> _wasm = [] {
    ...
    "getTotalThreadCount"_test = [] {
        ...
        expect(gt(count, 0UZ));
        expect(le(count, gr::thread_pool::thread::kThreadLimit));
    };
    ...
    "GR_MAX_WASM_THREAD_COUNT compile-time constant"_test = [] {
        ...
        expect(eq(static_cast<std::size_t>(GR_MAX_WASM_THREAD_COUNT), gr::thread_pool::thread::kThreadLimit));
#else
        expect(false) << "GR_MAX_WASM_THREAD_COUNT not defined";
#endif
#else
        expect(gt(gr::thread_pool::thread::kThreadLimit, 1UZ));
#endif
    };
    ...
    "Manager: WASM exhausting available threads"_test = [] {
        ...
        manager.replacePool(std::string(kDefaultIoPoolId), std::make_shared<ThreadPoolWrapper>(std::make_unique<BasicThreadPool>(kDefaultIoPoolId, TaskType::IO_BOUND, 1U, gr::thread_pool::thread::kThreadLimit), "CPU"));
        ...
        std::println("HW threads = {} - max wasm threads: {} actual: {} - pool max size: {}", //
            std::thread::hardware_concurrency(), gr::thread_pool::thread::kThreadLimit, gr::thread_pool::getTotalThreadCount(), pool->maxThreads());
        std::atomic<std::size_t> unexpectedExceptions{0UZ};
        std::atomic<std::size_t> expectedExceptions{0UZ};
        for (std::size_t i = 0UZ; i < gr::thread_pool::thread::kThreadLimit + 10UZ; ++i) {
            if (i >= (gr::thread_pool::thread::kThreadLimit - 10UZ)) {
    ...
    "computeDefaultThreadSplit respects invariants under edge conditions"_test = [] {
        ...
        const auto validate_split = [](std::size_t threadLimit, std::size_t reserve, std::source_location loc = std::source_location::current()) {
            ...
            const auto actual = s.cpuThreads + s.ioThreads;
            ...
            expect(le(s.cpuThreads, usable), loc) << std::format("limit={}, reserve={} -> CPU threads {} must be ≤ usable {}", threadLimit, reserve, s.cpuThreads, usable);
            expect(le(s.ioThreads, usable), loc) << std::format("limit={}, reserve={} -> IO threads {} must be ≤ usable {}", threadLimit, reserve, s.ioThreads, usable);
            expect(ge(s.cpuThreads, 1UZ), loc) << std::format("limit={}, reserve={} -> At least 1 CPU thread expected (got {})", threadLimit, reserve, s.cpuThreads);
            expect(ge(s.ioThreads, 1UZ), loc) << std::format("limit={}, reserve={} -> At least 1 IO thread expected (got {})", threadLimit, reserve, s.ioThreads);
    ...
    "Manager: respects thread limit and budget"_test = [] {
        ...
        const std::size_t threadCap = thread::kThreadLimit;
