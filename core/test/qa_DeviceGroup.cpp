...
/// records, in one shared log, when each member APPLIES a setting and when it RUNS. The group's contract is that
/// every member has applied before any member runs, so one change lands on the same chunk throughout the chain.
struct OrderProbe : gr::Block<OrderProbe> {
    gr::PortIn<float>  in;
    gr::PortOut<float> out;

    gr::Annotated<gr::Size_t, "marker"> marker = 0U;
    GR_MAKE_REFLECTABLE(OrderProbe, in, out, marker);

    static inline std::vector<std::string> log{}; // one group, one thread, one log

    void settingsChanged(const gr::property_map& /*oldSettings*/, const gr::property_map& newSettings) {
        if (newSettings.contains(std::string_view{"marker"})) {
            log.push_back(std::format("{} applied", name.value));
        }
    }

    [[nodiscard]] gr::work::Status processBulk(gr::InputSpanLike auto& input, gr::OutputSpanLike auto& output) {
        log.push_back(std::format("{} ran", name.value));
...
std::unique_ptr<Pipeline> makePipeline(std::size_t nMembers, bool reverseEmplacement) {
    ...
    auto domain = gr::makeDeviceGroup(std::move(inner));
...
const boost::ut::suite<"DeviceGroup"> _deviceGroupTests = [] {
    "makeDeviceGroup exports exactly the ports no interior edge claims, named after their member"_test = [] {
        gr::Graph inner;
    ...
    "makeDeviceGroup refuses two members that would export the same port name"_test = [] {
        gr::Graph inner;
        ...
        std::ignore = a;
        std::ignore = b;
        expect(gr::makeDeviceGroup(std::move(named)).has_value()) << "positive control: the same two members with distinct names are accepted";
    };
    ...
    "a group applies every member's staged settings before any member runs"_test = [] {
        OrderProbe::log.clear();

        gr::Graph inner;
        auto&     first  = inner.emplaceBlock<OrderProbe>({{"name", std::string("first")}});
        auto&     second = inner.emplaceBlock<OrderProbe>({{"name", std::string("second")}});
        expect(inner.connect(first, "out", second, "in").has_value());
        ...
        gr::Graph                      graph;
        auto&                          src     = graph.emplaceBlock<Source>({{"n_samples_max", gr::Size_t(8192)}});
        auto&                          snk     = graph.emplaceBlock<Sink>();
        const std::vector<std::string> inputs  = domain->inputs;
        const std::vector<std::string> outputs = domain->outputs;
        const auto&                    added   = graph.addBlock(std::move(domain->block));
        const std::string              name(added->uniqueName());
        expect(graph.emplaceEdge(std::string_view(src.unique_name), "out", std::string_view(name), inputs.at(0), gr::undefined_size, 0, "src->domain").has_value());
        expect(graph.emplaceEdge(std::string_view(name), outputs.at(0), std::string_view(snk.unique_name), "in", gr::undefined_size, 0, "domain->sink").has_value());
        ...
        // get samples into the group first: with externalStep nothing runs until it is asked to
        for (std::size_t warmUp = 0UZ; warmUp < 4UZ; ++warmUp) {
            std::ignore = src.work(256UZ);
            std::ignore = domainBlock->work(256UZ, gr::device::hostBackend());
        }

        // stage the same change on both members, then take ONE group work() call
        for (const std::shared_ptr<BlockModel>& member : domainBlock->graph()->blocks()) {
            std::ignore = member->settings().setStaged({{"marker", gr::Size_t(7)}});
        }
        OrderProbe::log.clear();
        std::ignore = src.work(256UZ);
        std::ignore = domainBlock->work(256UZ, gr::device::hostBackend());

        const auto firstRun     = std::ranges::find_if(OrderProbe::log, [](const std::string& entry) { return entry.ends_with("ran"); });
        const auto lastApplied  = std::ranges::find_last_if(OrderProbe::log, [](const std::string& entry) { return entry.ends_with("applied"); });
        const auto appliedCount = std::ranges::count_if(OrderProbe::log, [](const std::string& entry) { return entry.ends_with("applied"); });

        expect(eq(appliedCount, 2L)) << std::format("both members must apply the change, log: [{}]", gr::join(OrderProbe::log, ", "));
        expect(firstRun != OrderProbe::log.end()) << "a member must have run";
        if (firstRun != OrderProbe::log.end() && !lastApplied.empty()) {
            expect(lastApplied.begin() < firstRun) << std::format("every member must apply before ANY member runs, or one change lands on a different chunk per hop; log: [{}]", gr::join(OrderProbe::log, ", "));
        }
    };
    ...
    "an interior edge between two same-domain device members lives in memory the device can actually dereference"_test = [] {
        ...
        if (!gr::device::DeviceContextRegistry::instance().isServedExactly("gpu:sycl")) {
        ...
        auto domain = gr::makeDeviceGroup(std::move(inner));
        expect(domain.has_value()) << "makeDeviceGroup must succeed for two device-domain members joined by one interior edge";
        BlockModel* domainBlock = domain->block.get();
    ...
    "makeDeviceGroup leaves its members' settings alone"_test = [] {
        gr::Graph inner;
        ...
        auto domain = gr::makeDeviceGroup(std::move(inner));
        expect(domain.has_value());
        static_cast<gr::DeviceGroupWrapper*>(domain->block.get())->start();
    ...
    "makeDeviceGroup keeps a port private when asked, and exports the rest"_test = [] {
        gr::Graph inner;
        auto&     head = inner.emplaceBlock<Copy>({{"name", std::string("head")}});
        auto&     tail = inner.emplaceBlock<Copy>({{"name", std::string("tail")}});
        expect(inner.connect(head, "out", tail, "in").has_value());

        auto domain = gr::makeDeviceGroup(std::move(inner), {"tail:out"});
        expect(domain.has_value());
    ...
    "makeDeviceGroup refuses members spread across two device domains"_test = [] {
        gr::Graph mixed;
        ...
        auto refused = gr::makeDeviceGroup(std::move(mixed));
        ...
        expect(gr::makeDeviceGroup(std::move(allowed)).has_value()) << "a host member alongside one device domain is a legitimate mix";
    };
};
