...
struct DemoSubSchedulerResult {
    ...
    void setGraph(gr::Graph&& graph) {
        scheduler = std::unique_ptr<BlockModel>(std::make_unique<Wrapper>(std::move(graph)).release());
...
const boost::ut::suite BasicSchedulerWrapperTests = [] {
    ...
    "Instantiate scheduler wrapper"_test = [&] { SchedulerWrapper<Scheduler> scheduler{gr::Graph()}; };

    "Add scheduler wrapper to a graph"_test = [&] {
        auto scheduler = std::unique_ptr<BlockModel>(std::make_unique<SchedulerWrapper<Scheduler>>(gr::Graph()).release());
