...
class PortInfo {
    ...
    std::string                     _colour;        /// stored in BlockModel::uiConstraints()
...
public:
    ...
    [[nodiscard]] constexpr const char*          colour() const noexcept { return _colour.c_str(); }
    ...
    void colour(std::string_view colour) noexcept {
        _colour = colour;
        updateUIConstraints("colour", static_cast<std::string>(colour));
    }
...
[[nodiscard]] inline std::string_view colour(const gr::BlockModel& block) noexcept {
    const auto& ui = block.uiConstraints();
    return ui.contains("colour") ? std::string_view{std::get<std::string>(ui.at("colour"))} : std::string_view{};
}

[[nodiscard]] inline std::string_view colour(const gr::Edge& edge) noexcept {
    const auto& ui = edge.uiConstraints();
    return ui.contains("colour") ? std::string_view{std::get<std::string>(ui.at("colour"))} : std::string_view{};
}

inline void colour(const std::shared_ptr<gr::BlockModel>& block, std::string_view colour) { block->uiConstraints()["colour"] = std::string(colour); }
inline void colour(gr::Edge edge, std::string_view colour) { edge.uiConstraints()["colour"] = std::string(colour); }
template<gr::PortDirection direction>
void colour(const std::shared_ptr<gr::BlockModel>& block, gr::PortDefinition portDefinition, std::string_view colour, std::source_location loc = std::source_location::current()) {
    std::vector<PortInfo> infos   = getPortInfos<direction>(block);
    std::size_t           portIdx = gr::absolutePortIndex<direction>(block, portDefinition, loc);
    infos.at(portIdx).colour(colour);
}
...
static void layoutSugiyama(gr::Graph& graph, const LayoutPreference& config = {}, std::source_location loc = std::source_location::current()) {
    ...
    for (auto& loop : feedbackLoops) {
        ...
        std::println("A) feedback edge: {}", loop.edges.back());
    }
...
struct Canvas {
    ...
    void hSpan(std::size_t x0, std::size_t x1, std::size_t y, std::string_view colour = {}) {
        if (x0 > x1) {
            std::swap(x0, x1);
        }
        for (std::size_t x = x0; x < x1; ++x) {
            addMask(x, y, EAST, colour);
            addMask(x + 1UZ, y, WEST, colour);
        }
    }
    void vSpan(std::size_t y0, std::size_t y1, std::size_t x, std::string_view colour = {}) {
        if (y0 > y1) {
            std::swap(y0, y1);
        }
        for (std::size_t y = y0; y < y1; ++y) {
            addMask(x, y, SOUTH, colour);
            addMask(x, y + 1UZ, NORTH, colour);
        }
    }
...
template<gr::arithmetic_or_complex_like T = std::size_t>
void drawPath(Canvas& canvas, const gr::Edge& edge, const std::vector<Point<T>>& path, [[maybe_unused]] bool isFeedback = false) {
    const std::string_view colour_ = colour(edge);
    const std::size_t      n       = path.size();
    if (n < 2) {
        if (n == 1) { // single‐point marker
            canvas.put(path[0].x, path[0].y, "*", colour_);
        }
        return;
    }
    ...
    canvas.addPath(p0.x, p0.y, static_cast<std::byte>(dirs[1UZ]), std::hash<gr::Edge>{}(edge), colour_);
    ...
    for (std::size_t i = 1UZ; i + 1UZ < n; ++i) {
        ...
        canvas.addPath(p.x, p.y, static_cast<std::byte>(mask), std::hash<gr::Edge>{}(edge), colour_);
    }

    // draw the final stub at the destination cell (incoming edge only):
    auto& pt = path.back();
    canvas.addPath(pt.x, pt.y, static_cast<std::byte>(oppositeDirection(dirs.back())), std::hash<gr::Edge>{}(edge), colour_);
}
...
template<gr::arithmetic_or_complex_like T = std::size_t>
inline void drawBlock(Canvas& c, const std::shared_ptr<gr::BlockModel>& block) {
    using enum Side;
    assert(block.get() != nullptr);

    // draw the block's frame
    const std::string_view blockColour = colour(*block);
    const Point<T>         topLeft     = getPosition<T>(block);
    const T                width       = calculateWidth<T>(block);
    const T                height      = calculateHeight<T>(block);
    T                      x1          = topLeft.x + width - T(1);
    T                      y1          = topLeft.y + height - T(1);
    c.hSpan(topLeft.x, x1, topLeft.y, blockColour);
    c.hSpan(topLeft.x, x1, y1, blockColour);
    c.vSpan(topLeft.y, y1, topLeft.x, blockColour);
    c.vSpan(topLeft.y, y1, x1, blockColour);
    ...
    for (PortInfo& info : infos | std::views::join) {
        ...
        std::string_view colour = info.colour();
        const Point<T>   pt     = info.anchorPoint<T>();
        c.put(pt.x, pt.y, portGlyph(info, *info.preferredSide()), colour);

        switch (*info.preferredSide()) {
        case Left: c.put(pt.x + 1UZ, pt.y, info.name(), colour); break;
        case Right: c.put(pt.x - utf8Width(info.name()), pt.y, info.name(), colour); break;
        case Top: c.put(pt.x + 1UZ, pt.y, info.name(), colour); break;
        case Bottom: c.put(pt.x + 1UZ, pt.y, info.name(), colour); break;
        }
    }
}
...
inline static gr::Graph example_large() {
    ...
    colour(gr::graph::findBlock(graph, sum.unique_name).value(), "\033[32m");
    colour(gr::graph::findBlock(graph, src2.unique_name).value(), "\033[32m");

    for (auto& loop : gr::graph::detectFeedbackLoops(graph)) {
        colour(loop.edges.back(), utf8Feedback); // colour feedback edges
    }
    colour(gr::graph::findEdge(graph, "special edge").value(), "\033[01;35m"); // colour arbitrary edge

    colour<gr::PortDirection::OUTPUT>(gr::graph::findBlock(graph, m3s.unique_name).value(), {0UZ, 0UZ}, "\033[38;2;150;80;0m");
