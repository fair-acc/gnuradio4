...
const suite<"gr::complex"> tests =
    [] {
        ...
        "satisfies gr::meta::complex_like (keystone)"_test = [] {
            static_assert(gr::meta::complex_like<gr::complex<float>>);
            static_assert(gr::meta::complex_like<gr::complex<double>>);
            // std::complex still matches — behaviour-preserving
            static_assert(gr::meta::complex_like<std::complex<float>>);
            static_assert(gr::meta::complex_like<std::complex<double>>);
            expect(true);
        };
        ...
        "conversion policy A: implicit both ways, constexpr, lossless"_test = [] {
            // option A — both directions stay implicit for drop-in std::complex interop
            static_assert(std::is_convertible_v<std::complex<float>, gr::complex<float>>);
            static_assert(std::is_convertible_v<gr::complex<float>, std::complex<float>>);
            static_assert(std::is_convertible_v<std::complex<double>, gr::complex<double>>);
            static_assert(std::is_convertible_v<gr::complex<double>, std::complex<double>>);
            constexpr std::complex<float> sc{3.f, -4.f};
            constexpr gr::complex<float>  gc   = sc; // implicit
            constexpr std::complex<float> back = gc; // implicit
            static_assert(back.real() == 3.f && back.imag() == -4.f);
            expect(true);
        };

        "conversion policy A: mixed-type arithmetic resolves to gr::complex"_test = [] {
            // std::complex's operators are templates that don't deduce against gr::complex, so a
            // mixed same-precision expression binds gr's operator and implicitly converts the
            // std::complex operand — it compiles (no ambiguity) and yields gr::complex. pinned so a
            // later change to the conversions (e.g. making one explicit) is caught here.
            gr::complex<float>  g{1.f, 2.f};
            std::complex<float> s{3.f, 4.f};
            static_assert(std::is_same_v<decltype(g + s), gr::complex<float>>);
            static_assert(std::is_same_v<decltype(s + g), gr::complex<float>>);
            auto sum = g + s;
            expect(eq(sum.re, 4.f) and eq(sum.im, 6.f));
            std::complex<float> back = g + s; // result flows back to std::complex implicitly, lossless
            expect(eq(back.real(), 4.f) and eq(back.imag(), 6.f));
            // a fully-migrated (all gr::complex) expression compiles cleanly
            static_assert(requires(gr::complex<float> a, gr::complex<float> b) { a + b; a - b; a * b; a / b; });
        };
