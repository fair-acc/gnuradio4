...
struct TestContext {
    ...
    TestContext() : registry(gr::globalBlockRegistry()), loader(registry, std::vector<std::filesystem::path>{"core/test/plugins", "test/plugins", "plugins"}) {}
};
...
class RunnerContext {
public:
    RunnerContext() { context = std::make_unique<TestContext>().release(); }
