...
const boost::ut::suite PortApiTests = [] {
    ...
    "ConnectionApi - Graph-level"_test = [] {
        ...
        expect(eq(ConnectionResult::SUCCESS, flow.connect2(number, number.value, scaled, scaled.original)));
        expect(eq(ConnectionResult::SUCCESS, flow.connect2(scaled, scaled.scaled, added, added.addend0)));
        expect(eq(ConnectionResult::SUCCESS, flow.connect2(answer, answer.value, added, added.addend1)));

        expect(eq(ConnectionResult::SUCCESS, flow.connect2(added, added.sum, out, out.sink)));
