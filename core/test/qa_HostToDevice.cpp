...
const suite<"basic::HostToDevice"> h2dTests = [] {
    ...
    "a stream that is not a whole number of chunks still delivers its tail"_test = [] {
        // chunk_size is a floor, so the framework stops calling processBulk with 232 samples left; the epilogue is
        // the only thing that moves them, and a silent loss here would read as a slightly short but plausible run
        constexpr gr::Size_t N      = 1000;
        constexpr gr::Size_t kChunk = 256;
        gr::Graph            flow;
        auto&                src  = flow.emplaceBlock<gr::testing::CountingSource<float>>({{"n_samples_max", N}});
        auto&                h2d  = flow.emplaceBlock<gr::basic::HostToDevice<float>>({{"chunk_size", kChunk}});
        auto&                d2h  = flow.emplaceBlock<gr::basic::DeviceToHost<float>>();
        auto&                sink = flow.emplaceBlock<gr::testing::CountingSink<float>>({{"n_samples_max", N}});
        ...
        expect(eq(sink.count.value, N)) << "3 whole chunks plus a 232-sample tail must all arrive";
    };

    "chunk_size has the documented default"_test = [] {
