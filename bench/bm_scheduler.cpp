...
inline constexpr std::size_t N_BLOCKS  = 5;

template<typename T, char op>
class math_op : public fg::block<math_op<T, op>, fg::PortInNamed<T, "in">, fg::PortOutNamed<T, "out">> {
...
create_cascade(fg::graph &flow_graph, Sink &src, Source &sink, std::size_t depth = 1) {
    ...
    for (std::size_t i = 0; i < depth; i++) {
        mult1.emplace_back(std::addressof(flow_graph.make_block<multiply<T>>(T(2), fmt::format("mult.{}", i))));
        mult2.emplace_back(std::addressof(flow_graph.make_block<divide<T>>(T(2), fmt::format("div.{}", i))));
    }
...
test_graph_bifurcated(std::size_t depth = 1) {
    ...
    auto     &src   = flow_graph.make_block<test::source<T>>(N_SAMPLES);
    auto     &sink1 = flow_graph.make_block<test::sink<T>>();
    auto     &sink2 = flow_graph.make_block<test::sink<T>>();
...
[[maybe_unused]] inline const boost::ut::suite scheduler_tests = [] {
    ...
    fg::scheduler::breadth_first<multi_threaded> sched2_mt(test_graph_linear<float>(2 * N_BLOCKS), pool);
    ...
    fg::scheduler::simple<multi_threaded> sched3_mt(test_graph_bifurcated<float>(N_BLOCKS), pool);
    ...
    fg::scheduler::breadth_first<multi_threaded> sched4_mt(test_graph_bifurcated<float>(N_BLOCKS), pool);
    ...
    fg::scheduler::breadth_first<multi_threaded, profiler> sched4_mt_prof(test_graph_bifurcated<float>(N_BLOCKS), pool);
