...
inline const boost::ut::suite _constexpr_bm = [] {
...

    {
        ...
        filter.b          = fir_coeffs;
        auto merged_block = merge<"out", "in">(merge<"out", "in">(::test::source<float>(N_SAMPLES), std::move(filter)), ::test::sink<float>());
        //
        "merged src->fir_filter->sink"_benchmark.repeat<N_ITER>(N_SAMPLES) = [&merged_block]() { loop_over_work(merged_block); };
    }

    {
        ...
        auto merged_block = merge<"out", "in">(merge<"out", "in">(::test::source<float>(N_SAMPLES), std::move(filter)), ::test::sink<float>());
        //
        "merged src->iir_filter->sink - direct form I"_benchmark.repeat<N_ITER>(N_SAMPLES) = [&merged_block]() { loop_over_work(merged_block); };
    }

    {
        ...
        auto merged_block = merge<"out", "in">(merge<"out", "in">(::test::source<float>(N_SAMPLES), std::move(filter)), ::test::sink<float>());
        //
        "merged src->iir_filter->sink - direct form II"_benchmark.repeat<N_ITER>(N_SAMPLES) = [&merged_block]() { loop_over_work(merged_block); };
    }
...

    {
        ...
        auto     &src    = flow_graph.make_block<::test::source<float>>(N_SAMPLES);
        auto     &sink   = flow_graph.make_block<::test::sink<float>>();
        auto     &filter = flow_graph.make_block<fir_filter<float>>({ { "b", fir_coeffs } });
...

    {
        ...
        auto     &src    = flow_graph.make_block<::test::source<float>>(N_SAMPLES);
        auto     &sink   = flow_graph.make_block<::test::sink<float>>();
        auto     &filter = flow_graph.make_block<iir_filter<float, IIRForm::DF_I>>({ { "b", iir_coeffs_b }, { "a", iir_coeffs_a } });
...

    {
        ...
        auto     &src    = flow_graph.make_block<::test::source<float>>(N_SAMPLES);
        auto     &sink   = flow_graph.make_block<::test::sink<float>>();
        auto     &filter = flow_graph.make_block<iir_filter<float, IIRForm::DF_II>>({ { "b", iir_coeffs_b }, { "a", iir_coeffs_a } });
