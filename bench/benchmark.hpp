            ...
            const auto        add_statistics      = [&duration_s]<typename T>(ResultMap &map, const T &time_diff) {
                if constexpr (N_ITERATIONS != 1) {
                    ...
                    if (stddev == 0) {
                        ...
                        map.try_emplace("stddev", stddev, "s");
                    }
                    map.try_emplace("median", median, "s");
    ...
    print() {
        ...
        const auto        format          = [](auto &value, const std::string &unit) -> std::string {
            ...
            if (std::holds_alternative<std::monostate>(value)) {
                ...
                return fmt::format("{}", to_si_prefix(std::get<long double>(value), unit));
            } else if (std::holds_alternative<uint64_t>(value)) {
                return fmt::format("{}", to_si_prefix(std::get<uint64_t>(value), unit));
        ...
        for (auto &[test_name, result_map] : data) {
            for (auto &[metric_key, value] : result_map) {
                ...
                std::size_t value_size     = format(value.first, value.second).size();
