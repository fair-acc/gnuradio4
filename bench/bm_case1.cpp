...
public:
    ...
    explicit multiply(T factor, std::string name = fair::graph::this_source_location()) : _factor(factor) {
        this->set_name(name);
    }
...
class copy : public fg::node<copy<T, N_MIN, N_MAX, use_bulk_operation, use_memcopy>, fg::IN<T, "in", N_MIN, N_MAX>, fg::OUT<T, "out", N_MIN, N_MAX>, fg::limits<N_MIN, N_MAX>> {
public:
    template<fair::meta::t_or_simd<T> V>
    [[nodiscard]] constexpr T
    process_one(V a) const noexcept {
...
inline const boost::ut::suite _constexpr_bm = [] {
...

    {
        ...
        auto merged_node          = merge<"out", "in">(merge<"out", "in">(test::source<float>(N_SAMPLES), //
                                                                 test::cascade<10, decltype(gen_mult_block_float())>(gen_mult_block_float(), gen_mult_block_float)),
                                              test::sink<float>());
    ...
    auto src_mult_n_sink_SIMD                                                                            = merge<"out", "in">(merge<"out", "in">(test::source<float_simd>(N_SAMPLES),
                                                                      test::cascade<10, decltype(gen_mult_block_simd_float())>(gen_mult_block_simd_float(), gen_mult_block_simd_float)),
                                                   test::sink<float_simd>());
...
inline const boost::ut::suite _runtime_tests = [] {
...

    {
        ...
        "runtime   src->sink overhead"_benchmark.repeat<N_ITER>(N_SAMPLES) = [&flow_graph, &token]() {
            test::n_samples_produced = 0LU;
            test::n_samples_consumed = 0LU;
            expect(token);
...

    {
        ...
        "runtime   src->copy->sink"_benchmark.repeat<N_ITER>(N_SAMPLES) = [&flow_graph, &token]() {
            test::n_samples_produced = 0LU;
            test::n_samples_consumed = 0LU;
            expect(token);
            flow_graph.work(token);
...

    {
        ...
        "runtime   src->copy^10->sink"_benchmark.repeat<N_ITER>(N_SAMPLES) = [&flow_graph, &token]() {
            test::n_samples_produced = 0LU;
            test::n_samples_consumed = 0LU;
            expect(token);
            flow_graph.work(token);
...

    {
        ...
        "runtime   src(N=1024)->b1(N≤128)->b2(N=1024)->b3(N=32...128)->sink"_benchmark.repeat<N_ITER>(N_SAMPLES) = [&flow_graph, &token]() {
            test::n_samples_produced = 0LU;
            test::n_samples_consumed = 0LU;
            expect(token);
            flow_graph.work(token);
...

    {
        ...
        "runtime   src->mult(2.0)->mult(0.5)->add(-1)->sink"_benchmark.repeat<N_ITER>(N_SAMPLES) = [&flow_graph, &token]() {
            test::n_samples_produced = 0LU;
            test::n_samples_consumed = 0LU;
            expect(token);
            flow_graph.work(token);
...

    {
        ...
        "runtime   src->(mult(2.0)->mult(0.5)->add(-1))^10->sink"_benchmark.repeat<N_ITER>(N_SAMPLES) = [&flow_graph, &token]() {
            test::n_samples_produced = 0LU;
            test::n_samples_consumed = 0LU;
            expect(token);
            flow_graph.work(token);
