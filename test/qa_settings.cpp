...
template<typename T>
struct TestBlock : public node<TestBlock<T>, BlockingIO, TestBlockDoc, SupportedTypes<float, double>> { // TODO: re-enable the BlockingIO -> seems to trigger ASAN in multi-threaded mode ...
    IN<T>  in{};
    OUT<T> out{};
    // parameters
...
const boost::ut::suite SettingsTests = [] {
    ...
    "run-time type-erased node setter/getter"_test = [] {
        ...
        auto ioThreadPool = std::make_shared<fair::thread_pool::BasicThreadPool>("test_pool", fair::thread_pool::TaskType::IO_BOUND, 2_UZ);
        //
