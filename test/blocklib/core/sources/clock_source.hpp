...
ClockSource Documentation -- add here
    ...
    process_bulk(PublishableSpan auto &output) noexcept {
        ...
        const auto        remaining_samples  = static_cast<std::size_t>(n_samples_max - n_samples_produced);
        const std::size_t limit              = std::min(writableSamples, remaining_samples);
        const std::size_t n_available        = std::min(limit, static_cast<std::size_t>(chunk_size.value));
        ...
        while (next_tag < tags.size() && tags[next_tag].index <= static_cast<std::make_signed_t<std::size_t>>(n_samples_produced + n_available)) {
            ...
            samples_to_produce = static_cast<std::size_t>(static_cast<std::uint64_t>(tags[next_tag].index) - n_samples_produced);
            this->forward_tags();
            next_tag++;
        }
        samples_to_produce = std::min(samples_to_produce, static_cast<std::size_t>(n_samples_max.value));
